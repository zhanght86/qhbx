...
public class OrgMgr {
    ...
    public static String getSubOrgAndChildDeptsSql(String cmpyCode, String parentOrgCode) {
        ...
        sql.append(" and DEPT_TYPE = ").append(Constant.DEPT_TYPE_ORG); //机构
    ...
    public static List<DeptBean> getSubOrgs(String cmpyCode, String parentOrgCode) {
        ...
        sqlBean.and("DEPT_TYPE", Constant.DEPT_TYPE_ORG);
    ...
    public static DeptBean getParentOrg(String deptCode) {
	 ...
	 /**
	  * 获取指定层级的机构编码
	  * @param odeptPath 当前用户机构层级编码【CODE_PATH】
	  * @param odeptlevel 需要获取的层级
	  * @return 需要获取指定层级的机构编码
	  */
   public static String getOdeptlevelCode(String odeptPath, int odeptlevel) {
	   String[] odeptpath = odeptPath.split("\\^");
	   return odeptpath[odeptlevel - 1];
   }
    /**
     * 更新子部门信息
     * 
    ...
    public static void updateChildDepts(Bean oldBean, Bean newBean) {
        ...
        for (Bean deptBean : beanList) {
            ...
            if (deptBean.getInt("DEPT_TYPE") == Constant.DEPT_TYPE_ORG) { //自身为机构
                updateDept.set("TDEPT_CODE", deptBean.getId());
                updateDept.set("ODEPT_CODE", deptBean.getId());
            } else { //自身为部门，查找父部门类型
                if (newBean.getInt("DEPT_TYPE") == Constant.DEPT_TYPE_ORG) { //父部门为机构
    ...
    public static boolean checkServAuth(Bean servDef) {
        ...
        if (!Context.isDebugMode() && servDef.getInt("SERV_AUTH_FLAG") == Constant.AUTH_FLAG_SERV) { 
            Bean userState = Context.getOnlineUserState();
            if (userState == null) {
                hasAuth = false;
            } else {
