...
public class DataAclMgr {
    ...
    public static String getDataAclSql(String itemCode, String defCode) {
        ...
        //_IS_SERV_DACL_ADMIN 参数可以覆盖userBean.isAdminRole()权限
        if (Context.getThread("_IS_SERV_DACL_ADMIN", false)) {
            //如果参数指定当前用户是管理员，则不判断数据权限
            return "";
        } 
        
        if (Context.getThread("_IS_SERV_DACL_ADMIN") == null) {
            String roleName = Context.getSyConf("SERV_DACL_ADMIN", "");
            if (roleName.length() == 0) {
                if (userBean.isAdminRole()) { // 系统管理角色缺省不判断数据权限
                    return "";
                }
            } else if (userBean.existInRole(roleName)) { //指定权限管理员不判断权限
                return "";
            }
        }
        ...
        if (dataDef != null) {
            where.append(" and ").append(itemCode).append(" in (")
                ...
                .append(")").append(" and OWNER_SCOPE in (").append(getUserRightScope(userBean)).append("))");
        } else {
     ...
     * @return 用户权限范围
     */
    private static String getUserRightScope(UserBean userBean) {
        StringBuilder str = new StringBuilder();
        str.append("'ALL','").append(userBean.getODeptCode()).append("','");
        str.append(userBean.getTDeptCode()).append("'");
        if (!userBean.getTDeptCode().equals(userBean.getDeptCode())) {
            str.append(",'").append(userBean.getDeptCode()).append("'");
        }
