...
public class UserBean extends Bean {
    ...
    public String getDeptRoleUserStr() {
        ...
        sbUser.append("'U_").append(getCode()).append("','D_").append(getDeptCode())
            .append("'");
        if (!getDeptCode().equals(getTDeptCode())) {//所属部门
        	sbUser.append(",'D_").append(getTDeptCode()).append("'");
        }
        //所属机构
        sbUser.append(",'D_").append(getODeptCode()).append("'");
...
//        String[] deptArray = codePath.split(Constant.CODE_PATH_SEPERATOR);
    ...
    private Bean getUserExt() {
        ...
        if (userExt == null) {
            ...
            //设置群组信息
            Bean param = new Bean();
            param.set("USER_CODE", this.getCode()).set("S_FLAG", Constant.YES_INT)
                .set(Constant.PARAM_SELECT, "distinct GROUP_CODE, GU_ADMIN");
