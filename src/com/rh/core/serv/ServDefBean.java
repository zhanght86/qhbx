...
import com.rh.core.serv.ServUtils.SERV_DEF;
...
public class ServDefBean extends Bean {
    ...
    /**
     * 获取移动端的卡片模版；如不存在，则返回PC端的卡片模版
     * 
     * @return
     */
    public String getCardTmplContentMB() {
    	try {
	        if (!Context.isDebugMode() && this.contains("SERV_CARD_TMPL_CONTENT_MB")) {
	            return this.getStr("SERV_CARD_TMPL_CONTENT_MB");
	        }
	        int pos = this.getId().indexOf("_");
	        String path = Context.appStr(APP.SYSPATH) + this.getId().substring(0, pos).toLowerCase() + "/tmpl/"
	                + this.getCardTmplName()+"_MB" + ".html";
	        return FileHelper.readFile(path);
    	} catch (Exception e) {
    		log.info(e.getMessage());
    		return getCardTmplContent();
    	}
    }
     ...
     */
    public Bean getFirstSearchDef() {
        Bean searchDef = null;
        if (this.getInt("SERV_SEARCH_FLAG") == Constant.YES_INT) {
            if (this.contains("SY_SERV_SEARCH") && this.isNotEmpty("SY_SERV_SEARCH")) {
                searchDef = this.getBean("SY_SERV_SEARCH");
            } else {
                //使用引用服务的搜索配置
                searchDef = SearchDefUtils.getFirstSearchDefBean(this);
        ...
        if (whereList.size() > 0) {
            for (Bean whereBean : whereList) {
                if ((whereBean.getInt("S_FLAG") == Constant.YES_INT)
                    ...
                    switch (flowFlag) {
                    case Constant.FLOW_FLAG_ODEPT:
                      whereStr = " and OWNER_ID='" + userBean.getODeptCode() + "'";
                        break;
                    case Constant.FLOW_FLAG_TDEPT: //部门领导或同等权限
                        whereStr = " and OWNER_ID='" + userBean.getTDeptCode() + "'";
                        break;
                    case Constant.FLOW_FLAG_DEPT: //处室领导或同等权限
                        whereStr = " and OWNER_ID in ('" + userBean.getDeptCode() + "','" 
                                + userBean.getCode() + "')";
                        break;
                    case Constant.FLOW_FLAG_USER:
...
//                        where.append(" and ").append(this.getPKey())
     ...
     */
    public Object getDataCache(Object key) {
     ...
     */
    public void setDataCache(Object key, Object data) {
