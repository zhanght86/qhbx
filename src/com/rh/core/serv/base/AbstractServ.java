...
public abstract class AbstractServ extends BaseServ {
     ...
     *          boolean defFlag = paramBean.getBoolean(Constant.PARAM_DEL_FLAG);
     *      可以通过paramBean获取所有待删除数据的列表
     *          List<Bean> dataList = paramBean.getList(Constant.PARAM_BATCHDELS);
     */
     ...
     * @param paramBean 参数信息
     * @param outBean 输出信息 
     *      可以通过outBean.getOkDatas()获取所有已删除的数据对象信息
    ...
    /**
     * 单条记录显示之后的拦截方法，由子类重载(用于手机端卡片模板强行添加)
     * @param paramBean 参数信息
     * @param outBean 输出信息
     */
    ...
    protected String getFullWhere(ParamBean paramBean) {
        ...
        if (serv.getCmpy().length() > 0) { // 自动处理存在公司字段的过滤
            String cmpy = Context.getCmpy();
            if (cmpy.length() > 0) {
                where.append(" and ").append(serv.getCmpy()).append("='").append(cmpy).append("'");
            }
        }
     ...
     */
    public void appendUserItemInfo(String itemCode, Bean bean) {
