...
public class ServDefItem extends CommonServ {
    ...
    protected void beforeSave(ParamBean paramBean) {
        ...
        if (dataBean.getInt("ITEM_INPUT_TYPE") == Constant.ITEM_INPUT_TYPE_CHECKBOX
            || dataBean.getInt("ITEM_INPUT_TYPE") == Constant.ITEM_INPUT_TYPE_RADIO
            || dataBean.getInt("ITEM_INPUT_TYPE") == Constant.ITEM_INPUT_TYPE_SELECT) {
            dictFlag = true;
        } else if (dataBean.getInt("ITEM_INPUT_MODE") == Constant.ITEM_INPUT_MODE_DICT) {
            dictFlag = true;
        }
        if (dictFlag) { //需要修改字典项
        ...
        if (paramBean.contains("ITEM_LIST_FLAG")) { //调整列表显示自动调整对应的高级查询
            if (paramBean.getInt("ITEM_LIST_FLAG") == Constant.ITEM_LIST_FLAG_NO) {
    ...
    protected void afterDelete(ParamBean paramBean, OutBean outBean) {
        if (!paramBean.getLinkMode()) { //非级联删除模式才更新json文件
            ...
            if (dataList.size() > 0) {
                ...
                if (dataBean != null) {
                    ServUtils.clearServCache(dataBean.getStr("SERV_ID"));
                }
            }
        }
    }
