    ...
    protected void afterSave(ParamBean paramBean, OutBean outBean) {
        if (outBean.isOk() && !paramBean.getLinkMode()) { //非级联才更新缓存
            if (!outBean.isEmpty("LINK_ID")) {
                Bean link = ServDao.find(ServDefLink.SERV_ID_SERV_LINK, outBean.getStr("LINK_ID"));
                ServUtils.clearServCache(link.getStr("SERV_ID")); 
            }
        }
    }
        ...
        if (outBean.isOk() && !paramBean.getLinkMode()) { //非级联才更新缓存
            ...
            if (dataList.size() > 0) {
                ...
                if (dataBean != null && !dataBean.isEmpty("LINK_ID")) {
                    Bean link = ServDao.find(ServDefLink.SERV_ID_SERV_LINK, dataBean.getStr("LINK_ID"));
                    ServUtils.clearServCache(link.getStr("SERV_ID")); 
                }
            }
        }
