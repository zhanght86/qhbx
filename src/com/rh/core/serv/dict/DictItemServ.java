...
public class DictItemServ extends CommonServ {
    ...
    protected void afterSave(ParamBean paramBean, OutBean outBean) {
        if (outBean.isOk()) {
            ...
            if (!outBean.isEmpty(DictItemMgr.COL_DICT_ID) 
                    && !paramBean.getBoolean(Constant.IS_LINK_ACT)) { //非级联添加才更新缓存
     ...
     * @param dictId 字典ID
     */
    private void cleanDictCache(String dictId) {
        Bean dictDef = DictMgr.getDict(dictId);
        if (dictDef.getInt("DICT_IS_INNER") == Constant.YES_INT) {
            DictMgr.clearAllCache(dictId, true);
        } else {
    ...
    protected void afterDelete(ParamBean paramBean, OutBean outBean) {
        if (!paramBean.getBoolean(Constant.IS_LINK_ACT)) { //非级联删除模式才更新json文件
            ...
            if (dataList.size() > 0) {
                ...
                if (dataBean != null) {
                    cleanDictCache(dataBean.getStr(DictItemMgr.COL_DICT_ID));
                }
            }
        }
    }
