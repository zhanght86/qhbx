...
public class ParamBean extends Bean {
    ...
    public ParamBean(HttpServletRequest request) {
        super(JsonUtils.toBean(request.getParameter("data")));
        //获取其他参数信息
    ...
    public ParamBean setNowPage(int curPage) {
        ...
        this.set(Constant.NOWPAGE, curPage);
        return this;
    }
     ...
     * @return 分页信息
     */
    public Bean getQueryPage() {
        Bean page;
        if (this.contains(QUERY_PAGE)) {
            page = this.getBean(QUERY_PAGE);
        } else { //初始化页面配置，设定缺省值
            page = new Bean();
            page.set(Constant.NOWPAGE, 1);  
    ...
    /**
     * 
     * @return 从前台上传的PageBean实例数据
     */
    public PageBean getPageBean() {
        Bean bean = this.getQueryPage();

        return new PageBean(bean);
    }
    ...
    public ParamBean setQueryPageNowPage(int curPage) {
        Bean page = this.getQueryPage();
        page.set(Constant.NOWPAGE, curPage);
        return this;
    }
    ...
    public String getQueryPageOrder() {
        return this.getQueryPage().getStr("ORDER");
    }
    ...
    public ParamBean setQueryPageOrder(String order) {
        this.getQueryPage().set("ORDER", order);
        return this;
    }
    ...
    */
   public UserBean getDoUserBean() {
       // 获取 委托人 的 UserBean
       if (!this.isEmpty(Constant.AGENT_USER)) {
           String realUserId = this.getStr(Constant.AGENT_USER);
