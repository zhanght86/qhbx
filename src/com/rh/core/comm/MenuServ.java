...
public class MenuServ extends CommonServ {
    ...
    protected void afterSave(ParamBean paramBean, OutBean outBean) {
        if (outBean.isOk()) {
            ...
            UserMgr.clearMenuByCmpy(fullBean.getStr("S_CMPY"));
        }
    }
    ...
    protected void afterDelete(ParamBean paramBean, OutBean outBean) {
        ...
        if (okList.size() > 0) { // 清除用户菜单时间
            ...
            UserMgr.clearMenuByCmpy(menu.getStr("S_CMPY"));
        }
    }
            ...
            try {
                ...
                while (true) {
                	...
                	if (entry.getName().equals("SY_MENU_ACL.json")) {
                		...
                		log.debug("menuAclList: " + JsonUtils.toJson(menuAclList));
                		break;
                	}
                }                
                ...
                while (true) {
                	...
                	if (entry.getName().equals("SY_MENU_DATA.json")) {
                		...
                		log.debug("menuDataList: " + JsonUtils.toJson(menuDataList));
     ...
     * @return Bean 
     */
    public static int importMenu(List<Bean> menuList, List<Bean> menuAclList, String servId, String pId) {
        boolean pubFlag = servId.indexOf("PUBLIC") > 0 ? true : false; //公共服务标志为true 
        int count = 0;
        
        HashMap<String, List<Bean>> aclMap = getAclMap(menuAclList);
    ...
    private static String getMenuSaveRootPath() {
        final String webDoc = Context.appStr(APP.WEBINF_DOC);
