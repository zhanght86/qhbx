...
public class FavoriteServ extends CommonServ {
    ...
    protected void afterSave(ParamBean paramBean, OutBean outBean) {
        ...
        ParamBean query = new ParamBean(ServMgr.SY_COMM_MARK, ServMgr.ACT_FINDS);
        ...
        for (String tag : tags) {
            ...
            if (list.size() > 0) { //如果已存在标签，取出 MARK_ID
                ...
                OutBean out = ServMgr.act(ServMgr.SY_COMM_MARK, ServMgr.ACT_SAVE, markBean);
        ...
        if (list.size() > 0) {
            ...
            for (Bean bean : list) {
                ...
                if (out.getDataList().size() == 0) {
                    ...
                    ServDao.deletes(ServMgr.SY_COMM_MARK, markBean);
                }
            }
        }
    }
