...
public class PortalTemplServ  extends CommonServ implements PortalListenInterface {
    ...
    /** 个人门户 */
    public static final String PT_TYPE_USER_CODE = "5";
    ...
    public Bean getPortal(ParamBean paramBean) {
        ...
        try {
            outBean = new Bean();
            //参数监听处理
            String expStr = paramBean.getStr("PT_EXP"); //对应规则设定参数
            if (expStr.length() > 0) {
    ...
    protected void afterDelete(ParamBean paramBean, OutBean outBean) {
          ...
          for (Bean data : dataList) {
              if (data.getStr("PT_TYPE_DEFAULT").equals("2") && data.getStr("PT_TYPE").equals(PT_TYPE_USER_CODE)
    ...
    private PortalListenInterface createListener(String listener) {
        ...
        return (PortalListenInterface) ObjectCreator.create(PortalListenInterface.class, listener);
    }
    /**
     * 参数传递之前的拦截方法，由子类重载
     * @param paramBean 参数信息
