...
public class FormMindView {
     ...
     * @return 输出意见列表
     */
    public Bean outputMB(String odeptCode, boolean canCopy, boolean userDoInWf, ParamBean paramBean) {
        Bean bean = new Bean();
        bean.set("userMind", userMind);
        bean.set("mindTypeList", userMind.getMindTypeList());
        bean.set("sortType", sortType);
        ...
        bean.set("canCopy", canCopy); //设置是否显示复制的按钮
        bean.set("userDoInWf", userDoInWf); //设置是否显示复制的按钮
        ...
        // 手机端循环将意见的数字签名图片的路径拿出来
        for (int i = 0; i < userMindList.size(); i++) {
			Bean userMindBean = (Bean)userMindList.get(i);
			String signFile = userMindBean.getStr("SIGN_FILE");
			String imgSrc = "";
			...
			userMindBean.set("imgSrc", imgSrc);
		}
    ...
    public Bean mindListTitleMB(String odeptCode, ParamBean paramBean) {
        ...
        // 手机端增加意见列表数据
        List<Bean> userMindList = userMind.getMindList();
        // 手机端循环将意见的数字签名图片的路径拿出来
        for (int i = 0; i < userMindList.size(); i++) {
			Bean userMindBean = (Bean)userMindList.get(i);
			String signFile = userMindBean.getStr("SIGN_FILE");
			
			String oDeptCode = userMindBean.getStr("S_ODEPT");
			userMindBean.set("ODEPT_NAME", OrgMgr.getDept(oDeptCode).getName());
