...
public class MindLabelContentProvider {
    ...
    private HashMap<String, Bean> filterMindLabelItems(String servId) {
        ...
        for (String key : allItems.keySet()) {
            ...
            if (item.getInt("ITEM_TYPE") == Constant.ITEM_TYPE_DEFINE
                    && item.getInt("ITEM_INPUT_TYPE") == Constant.ITEM_INPUT_TYPE_LABEL
    ...
    private String getHtmlContent(Bean mindConfig) {
        ...
        bean.set("mindList", replaceReg(showList));
     ...
     * @return
     */
    private List<Bean> replaceReg(List<Bean> mindList) {
    	List<Bean> rtnList = new ArrayList<Bean>();
    	if (mindList != null) {
    		String mindContent = "";
    		for (Bean mind : mindList) {
    			mindContent = mind.getStr("MIND_CONTENT");
    			mindContent = mindContent.replaceAll("\n", "<br/>");
    			mindContent = mindContent.replaceAll("\r", "<br/>");
				// 将默认的最后一个回车符去掉
				if (mindContent.indexOf("<br/>") >= 0 && "<br/>".equals(mindContent.substring(
						mindContent.length() - 5, mindContent.length()))) {
					mindContent = mindContent.substring(0, mindContent.length() - 5);
				}
        		mind.set("MIND_CONTENT", mindContent);
        		rtnList.add(mind);
        	}
    	}
    	return rtnList;
