...
public class UserMind {
    ...
    public List<Bean> getMindListByType(String type, String odeptCode) {
        ...
        for (int i = 0; i < mindList.size(); i++) {
            ...
            if (odeptCode.equals(mindBean.getStr("S_ODEPT"))) {
                String mindCode = mindBean.getStr("MIND_CODE");
                if ((mindCode.startsWith(type1) || mindCode.startsWith(type2)) && canView(mindBean)) {
                    appendFileID(mindBean);
                    rtnList.add(mindBean);
                }
            }
     ...
     * @return 意见列表
     */
    public List<Bean> getMindListByMindSort(String odeptCode) {
        List<Bean> rtnList = new ArrayList<Bean>();
        //获取意见
        for (int i = 0; i < mindList.size(); i++) {
            Bean mindBean = mindList.get(i);
        ...
        //根据意见类型获取意见的排序
        for (Bean mindBean : rtnList){
        	Bean mindType = ServDao.find("SY_COMM_MIND_CODE", new Bean()
        		.set("CODE_ID", mindBean.getStr("MIND_CODE")));
        	mindBean.set("CODE_SORT",mindType.get("CODE_SORT"));
        }
        //进行排序
        for (int i = 0; i < rtnList.size() - 1; i++) {  
            for (int j = 1; j < rtnList.size() - i; j++) {  
                Integer a = (Integer) rtnList.get(j - 1).get("CODE_SORT");  
                Integer b = (Integer) rtnList.get(j).get("CODE_SORT");  
                if (a.compareTo(b) > 0) { // 比较两个整数的大小  
                    Bean mind = rtnList.get(j - 1);  
