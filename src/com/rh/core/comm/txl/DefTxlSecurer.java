 ...
 * 
 */
public class DefTxlSecurer extends AbstractTxlSecurer {
    ...
    public boolean isDefSecured(Bean userBean) {
        ...
        String deptCodePath = usBean.getStr("CODE_PATH");
        if (!"".equals(deptCodePath)) {
            deptCodePath = deptCodePath.substring(0, deptCodePath.length() - 1);
        }
        String dept = userBean.getStr("DEPT_CODE");
        if (deptCodePath.contains(dept)) {
            return true;
        }
        return false;
    }
