...
public class WorkTime {
    ...
    public long calWorktime(String cmpyId , String beginTime , String endTime) {
        ...
        // 是否启用节假日判断，默认不启用
        if ("false".equals(Context.getSyConf("BNOA_WORKDAY_ENABLE", "false"))) {
        	// 相差毫秒数
        	long tsCount = DateUtils.getDiffTime(beginTime, endTime);
        	long mins = tsCount / (60 * 1000);
        	if (tsCount % (60 * 1000) >= 30) {
        		// 不少于30秒，则算成1分钟
        		mins += 1;
        	}
        	return mins;
