 ...
 * 
 */
public class WfTimeOutJob implements RhJob {
	...
	private void doTuiHui(Bean procInst, Bean nodeInst) {
		if (nodeInst.isNotEmpty("PRE_NI_ID")) { //前面的节点不为空
			...
			String preNiId = nodeInst.getStr("PRE_NI_ID");
			
			Bean preNiBean = WfNodeInstDao.findNodeInstById(preNiId);
			
			String nextNodeCode = preNiBean.getStr("NODE_CODE");
			...
			String toUserCode = preNiBean.getStr("DONE_USER_ID"); //上个节点的办理人
			...
			wfAct.toNextForTuiHui(nextNodeCode, doneUserCode, toUserCode);
		}
	}
	...
	private void addRemind(String procName, String nodeName, Bean entity, 
		...
		if (remindCount == 0) { //之前还没提醒过， 第一次添加
            ...
            addRemindExt(procName, nodeName, entity,
    			...
    			if (DateUtils.getDiffTime(nextTimeOut, DateUtils.getDatetime(), DateUtils.FORMAT_DATETIME) >= 0) {
    				addRemind(entity, remindType, userTodo);
    				addRemindExt(procName, nodeName, entity,
    						remindType, userTodo, procInstId, nodeInstId, Constant.YES_INT, getNextTimeOut(emergenBean));
    	    		//修改领导entity提醒内容
