...
import com.rh.core.comm.zhidao.AnswerServ;
import com.rh.core.comm.zhidao.QuestionServ;
import com.rh.core.comm.zhidao.ZhidaoShareServ;
...
public class ZhidaoServ extends CommonServ {
    ...
    /*public OutBean getInviteMeAskList(ParamBean paramBean) {
        ...
        if(page == 0) {
        	...
        	questionQuery.setQueryPageNowPage(page);
        }
        if(count == 0) {
        	questionQuery.setQueryPageShowNum(10);
        }else{
        	questionQuery.setQueryPageShowNum(count);
        }
        String where = "";
        if (noAnswer != null && noAnswer.length() > 0) {
            where += " and Q_ANSWER_COUNTER = 0";
...
//    	int page = paramBean.getInt("page");
        ...
        //查询出所有邀请当前用户的记录
        ParamBean inviteQuery = new ParamBean();
        inviteQuery.setQueryExtWhere(" and TARGET_USER = '" + userId + "'");
        inviteQuery.setServId(ServMgr.SY_COMM_ZHIDAO_INVITE);
        inviteQuery.setOrder(" S_ATIME DESC");
        inviteQuery.setAct(ServMgr.ACT_QUERY);
        List<Bean> inviteList = ServMgr.act(inviteQuery).getDataList();
        for (int i = 0; i<inviteList.size(); i++) {
        	Bean inviteBean = inviteList.get(i);
        	try {
        		Bean quBean = ServMgr.act(ServMgr.SY_COMM_ZHIDAO_QUESTION, ServMgr.ACT_BYID,
    					new ParamBean().setId(inviteBean.getStr("Q_ID")));
    			inviteBean.set("Q_TITLE", quBean.getStr("Q_TITLE"));
			} catch (Exception e) {
				inviteList.remove(i);
				i--;
			}
        ...
        return new OutBean().setData(inviteList);
    }
    
    /**
     * 获取所有对于某个问题我邀请回答的人的列表		和		他们是否已经回答了该问题
     * @param paramBean - 问题ID,userID
     * @return - 邀请回答的人的列表		和		回答状态
     */
    public OutBean getMyInviteTargetList(ParamBean paramBean) {
    	String qId = paramBean.getStr("Q_ID");
    	String userId = paramBean.getStr("USER_CODE");
    	ParamBean query = new ParamBean(ServMgr.SY_COMM_ZHIDAO_INVITE, ServMgr.ACT_QUERY);
    	query.setQueryExtWhere(" and Q_ID = '" + qId + "' and S_USER = '" + userId + "'");
    	query.setQueryNoPageFlag(true);
    	List<Bean> list = ServMgr.act(query).getDataList();
    	for (int i = 0; i < list.size(); i++) {
			String userCode = list.get(i).getStr("TARGET_USER");
			Bean bean = ServDao.find(ServMgr.SY_COMM_ZHIDAO_ANSWER, new Bean()
					.set("Q_ID", qId).set("S_USER", userCode));
			if (bean != null && !("".equals(bean))) {
				list.get(i).set("flag", "answered");
			} else {
				list.get(i).set("flag", "noAnswer");
			}
		}
    ...
    public OutBean topStatistics(ParamBean paramBean) {
        return  zhidaoTopStatistics(paramBean);
    }
    ...
    public OutBean topStatisticsByChnl(ParamBean paramBean) {
        return zhidaoTopStatisticsByChnl(paramBean);
    }
...
//        for (Bean data : dataList) {
    ...
    public OutBean getThreeSpec(ParamBean param) {
        ...
        if (deptList.size() > 0) {
            ...
            if (deptAnswer.getDataList().size() > 0) {
                ...
                deptTitle = Lang.getSummary(deptTitle, 17);
        ...
        if (busiList.size() > 0) {
            ...
            if (busiAnswer.getDataList().size() > 0) {
                ...
                busiTitle = Lang.getSummary(busiTitle, 17);
    ...
    public OutBean getWorkmateShare(ParamBean paramBean) {
        ...
        for (int i = 0; i < shareList.size(); i++) {
            ...
            if ("SY_COMM_ZHIDAO_ANSWER".equals(share.getStr("SERV_ID"))) {
                ...
                shareList.get(i).set("Q_TITLE", answerOutBean.getStr("Q_TITLE"));
                shareList.get(i).set("A_ID", answerOutBean.getStr("A_ID"));
                shareList.get(i).set("A_CONTENT", Lang.getSummary(answerOutBean.getStr("A_CONTENT"), 80));
            } else {
				String qId = shareList.get(i).getStr("DATA_ID");
				OutBean questionOutBean = new QuestionServ().byid(new ParamBean().setId(qId));
				Bean answerOutBean = (Bean) getAnswer(
						new ParamBean().set("Q_ID", qId).set("summaryLength",
								80)).get("bestAnswer");
				
				shareList.get(i).set("Q_ID", questionOutBean.getStr("Q_ID"));
                shareList.get(i).set("Q_TITLE", questionOutBean.getStr("Q_TITLE"));
                shareList.get(i).set("A_ID", answerOutBean.getStr("A_ID"));
                shareList.get(i).set("A_CONTENT", answerOutBean.getStr("A_CONTENT"));
            }
        }
    ...
    public OutBean zhidaoTopStatistics(ParamBean paramBean) {
        ...
        return new IntegralServ().topStatistics(paramBean);
    }
    ...
    public OutBean zhidaoTopStatisticsByChnl(ParamBean paramBean) {
        paramBean.set("servId", ZhidaoServ.SY_COMM_ZHIDAO);
        return new IntegralServ().topStatisticsByChnl(paramBean);
    }
    ...
    /**
     * 获取所有的知道意见反馈信息
     * @param param - 参数，暂时不用传
     * @return - 意见反馈信息集
     */
    /*public List<Bean> getFeedBackList(ParamBean param) {
    	return new ZhidaoFeedBackServ().getFeedBackList(param);
    }*/
    
    /**
     * 根据问题的ID查询出问题的详细信息
     * @param param - 用,隔开的问题ID
     * @return - 问题详细信息的集合
     */
    public OutBean getQuestionByIds(ParamBean param) {
    	String[] ids = param.getStr("ids").split(",");
    	List<Bean> quesList = new ArrayList<Bean>();
    	for (int i = 0; i < ids.length; i++) {
			quesList.add(ServDao.find("SY_COMM_ZHIDAO_QUESTION", ids[i]));
		}
    	return new OutBean().setData(quesList);
    }
    ...
    /**
     * 判断某个userCode是否是当前登录用户已经关注过的人
     * @param param - userCode被查人ID
     * @return - 是否已经关注此人:true是(已关注此人),false否(未关注此人)
     */
    public OutBean isMyFollowPerson(ParamBean param) {
    	String userCode = param.getStr("USER_CODE");
    	String loginUser = Context.getUserBean().getCode();
    	Bean bean = new Bean();
    	bean.set("SERV_ID", "SY_COMM_ZHIDAO_PERSON_FOLLOW");
    	bean.set("USER_CODE", loginUser);
    	bean.set("DATA_ID", userCode);
    	Bean result = ServDao.find(ServMgr.SY_COMM_FOLLOW, bean);
    	OutBean outBean = new OutBean();
    	if (result != null) {
    ...
    /**
     * 获取我举报的信息
     * @param param - 我的userId
     * @return - 举报信息和相关信息集合
     */
    public OutBean getMyReportList(ParamBean param)	{
    	String userId = param.getStr("userId");
    	ParamBean query = new ParamBean();
    	/*query.setServId(ServMgr.SY_COMM_REPORT);*/
    	query.setAct(ServMgr.ACT_QUERY);
    	query.setQueryExtWhere(" and SERV_GROUP='SY_COMM_ZHIDAO' and USER_CODE = '" + userId + "'");
    	query.setQueryNoPageFlag(true);
    	List<Bean> reportList = ServMgr.act(query).getDataList();
    	for (int i = 0; i < reportList.size(); i++) {
			Bean report = reportList.get(i);
			String dataId = report.getStr("DATA_ID");
			if (report.getStr("SERV_ID").equals("SY_COMM_ZHIDAO_QUESTION")) {
				report.set(
						"dataDetail",
						ServMgr.act(ServMgr.SY_COMM_ZHIDAO_QUESTION,
								ServMgr.ACT_BYID, new ParamBean().setId(dataId)));
			} else {
				report.set(
						"dataDetail",
						ServMgr.act(ServMgr.SY_COMM_ZHIDAO_ANSWER,
								ServMgr.ACT_BYID, new ParamBean().setId(dataId)));
			}
		}
    	return new OutBean().setData(reportList);
    }
     ...
     * @return - 举报信息和相关信息集合
     */
    public OutBean getReportMeList(ParamBean param)	{
    	String userId = param.getStr("userId");
    	ParamBean query = new ParamBean();
    	/*query.setServId(ServMgr.SY_COMM_REPORT);*/
    	query.setAct(ServMgr.ACT_QUERY);
    	query.setQueryExtWhere(" and SERV_GROUP='SY_COMM_ZHIDAO' and DATA_OWNER = '" + userId + "'");
    	query.setQueryNoPageFlag(true);
    	List<Bean> reportList = ServMgr.act(query).getDataList();
    	for (int i = 0; i < reportList.size(); i++) {
			Bean report = reportList.get(i);
