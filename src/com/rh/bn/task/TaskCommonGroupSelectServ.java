 ...
 *
 */
public class TaskCommonGroupSelectServ extends CommonServ{
    @Override
    protected void afterQuery(ParamBean paramBean, OutBean outBean) {
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            bean.set("GROUPS_DESCRIPTION", "群组选择方案");
        }
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            Bean bean = new Bean();
            bean.set("GROUPS_NAME", deptBean.get("DEPT_NAME"));
            bean.set("GROUP_ID", "DEPT-" + deptBean.get("DEPT_CODE"));
            bean.set("GROUP_ORDER", deptBean.get("DEPT_SORT"));
            bean.set("GROUPS_DESCRIPTION", "部门");
            result.add(bean);
        }
