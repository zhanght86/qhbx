...
public class AddressServ extends CommonServ {
    ...
    protected void beforeQuery(ParamBean paramBean) {
        //获取当前用户上下级部门code值得sql语句
        paramBean.setQueryExtWhere(getOdeptCodes(Context.getUserBean()));
    }
    ...
    public OutBean createTree(ParamBean paramBean) {
        ...
        if("24".equals(odeptCode)){
            ...
            node.set("CHILD", childNodes).set("NAME", "总部").set("PID", 24).set("DEPT_TYPE", 2).set("ID", 12)
                    .set("LEAF", 2).set("FLAG", 1);
