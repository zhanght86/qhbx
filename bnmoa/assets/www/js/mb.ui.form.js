...
mb.ui.linkSelect.prototype.setValue = function(fileData) {
	...
	for (i = 0; i< fileData.length;i++) {
		...
		if(item["RELATE_TYPE"] == this.itemCode){
			$("<a href='"+FireFlyContextPath + "/file/" + item.RELATE_DATA_ID + "?mobile=1' class='sc-file-link' data-ajax='false' data-filename='"+item["RELATE_DATA_ID"]+"'>" + item["TITLE"] + "</a>").appendTo(this.obj);
		}
	}
};
