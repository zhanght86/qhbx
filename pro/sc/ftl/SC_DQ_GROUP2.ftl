	...
	<div class='dq-tab-wrapper'>
		<ul class='tabControl' id='tabControl'>
            <li class='dq-tab-item current'>
                <a href='#js-zbsh'> 支部生活 </a>
            </li>
            <li class='dq-tab-item'>
                <a href='#js-zrrd'> 早日入党</a>
            </li>
        </ul>
	</div>
	<div id="js-zbsh" class='portal-box-con-dq dq-content dq-tab-h clearfix' >
		<#if _DATA_0._DATA_?size !=0 >
			...
			<a href="javascript:void(0);" onclick="openDqListMoreByChnl('${_DATA_0._DATA_[0].CHNL_ID}','支部生活')" class="dq-more-bottom dq-red">更多&gt;&gt;</a> 
		<#else>
			 暂无内容! 
		</#if>
	</div>
	<div id="js-zrrd" class='portal-box-con-dq dq-content dq-tab-h clearfix' style="display:none;">
		<#if _DATA_1._DATA_?size !=0 >
			...
			<#list _DATA_1._DATA_ as obj>
					<li>
						<div class="news-title">
	                    	<span class="icon"></span>
							<a id = '${CHNL_ID3}${obj.NEWS_ID}' href='/cms/SY_COMM_INFOS/${obj.NEWS_ID}.html' data-id="${obj.NEWS_ID}" target="_blank">
							<span class="elipd" >${obj.NEWS_SUBJECT}</span>
							<script>
								var newQx = "${NEW_QX!'0'}";
								if (rhDate.doDateDiff('D',"${obj.NEWS_TIME?substring(0,10)}",rhDate.getCurentTime().substring(0, 10))<=newQx) {
									$("<i class='new'></i>").appendTo($("#${CHNL_ID3}${obj.NEWS_ID}"));
								}
							</script>
							</a>
	                    </div>
			...
			<a href="javascript:void(0);" onclick="openDqListMoreByChnl('${_DATA_1._DATA_[0].CHNL_ID}','早日入党')" class="dq-more-bottom dq-red">更多&gt;&gt;</a> 
		<#else>
			 暂无内容! 
		</#if>
	</div>
