...
<style type="text/css">
	.info-column{width:90%;margin-left:12px;}
	...
	a.readMore:hover{color: #dc0000;}
</style>
<div id='SC_COMM_CMS_TMPL-${$CHNL_ID$}' class='portal-box pt-gsyw-wrapper' style="width:98%;margin-left:-1px;">
			...
			<#if content_index == 0>
				<div id='first-new-${$CHNL_ID$}' class='first-new-${$CHNL_ID$}' isPaused="${content.IS_PAUSED}">
					...
					<div id='first-new-title-${$CHNL_ID$}' class='first-new-title-${$CHNL_ID$}'>
						...
						<#if (content.NEWS_SHORT_TITLE?length >40)>
								${content.NEWS_SHORT_TITLE?substring(0,40)}...
							<#else>
			...
			<#else>
				<tr>
					...
					<td  style="width:66%;position:relative">
						<a id='${$CHNL_ID$}${content.NEWS_ID}' title='${content.NEWS_SUBJECT}' style="width:100%;margin-left:3px;display: block;height:20px;" href="javascript:void(0);" onclick="newsView('${content.NEWS_ID}')">
							<span class="elipd" style="max-width:97%;">
