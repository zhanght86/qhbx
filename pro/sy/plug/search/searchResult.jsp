...
<%@ page import="com.rh.core.plug.search.AttachmentBean" %>
...
<script src="/oa/zh/fullSearch.js" type=text/javascript></script>
...
OutBean outBean = (OutBean)request.getAttribute(Constant.RTN_DISP_DATA);
 ...
 //判断终端设备
 boolean isMobileServer =   Context.appBoolean("MOBILE");
 //移动版地址
 String mobileOAaddr = Context.getSyConf("MB_OA_HOST_ADDR", "http://oa.cic.cn:8888");
 //pc版地址
 String pcOaAddr = Context.getSyConf("OA_HOST_ADDR_SEARCH","http://cicoa.cic.inter/");
 //加载左侧已选过滤条件
...
<body dir="ltr" id="gsr" class="" topmargin="3" lang="zh-Hans" marginheight="3">
			...
			<div id="cnt">
			   <div class="clearfix rh-search-first-line">
				      ...
				      <div id="sfcnt" class="rh-search-first-line-right">
				      	...
				      	<%final String filterCacheStr= JsonUtils.toJson(filterList);%>
				      	<input type="hidden" id="filterCache" name="filterCache" value='<%=filterCacheStr%>' />
						<form id="searchForm" action="/SY_PLUG_SEARCH.query.do" method="post" onsubmit="Search.search()" role="search">
							<div class="tsf-p" style="padding-bottom: 2px;">
								...
								<div id="suggest" style="display: none; z-index: 10;height:270px"></div></td>
							</div>
						</form>
					</div>
				</div>
						...
						<div id="search" class="rh-res">
							<div id="ires">
								<ol id="rso">
									<li class="rh-res-li">
									   ...
									   int docIndex = 0;
									   		for(Bean bean : dataList) {
									   			docIndex++;
									   		String service = bean.getStr(RhSearchClient.SOLR_SERVICE);
									   		String abstractText = bean.getStr(RhSearchClient.SOLR_ABSTRACT);
									   		...
									   		String title = bean.getStr(RhSearchClient.SOLR_TITLE);
									   		String displayTitle = title;
									   		String content = bean.getStr(RhSearchClient.SOLR_CONTENT);
									   		String url =  bean.getStr(RhSearchClient.SOLR_URL);
									   		String id = bean.getStr(RhSearchClient.SOLR_ID);
									   		String contFile = bean.getStr("file_path_strfield");
									   		List<AttachmentBean> attList = (List<AttachmentBean>)bean.get(RhSearchClient.SOLR_ATTACHMENT);
									  		int flag = 1;
									  	...
									  	%>
											...
											<span class="tl rh-serach-title" id="<%=id%>"> <span><%=serviceName%></span> &nbsp; <%
												//标题       
				  										...
				  										if (id.startsWith("http://")) {
				  											...
				  											if(url.startsWith("http://")){
				  											    //区分移动版
				  												if(isMobileServer){
				  													url = url.replace(pcOaAddr, mobileOAaddr);
				  												} 
				  												 out.println(" <a target='_blank' href='" + url + "'  class='rh-res-title' >" + displayTitle + "</a> ");
				  												} else {
												...
												//删除
												if (manager) {
													 out.println("<a class='rhSearch-load-a' title='删除索引' href=\"javascript:Search.deleteIndex('" + id +"')\"'>删除</a>");
												}
											...
											<div style="color: block" class="s">
												<div class="rh-contentPreview">
													<%
										      			 ...
										      			 List<String> relativeList = (List) bean.get(RhSearchClient.SOLR_RELATIVE);
												...
												<% 
												 ...
												 for (AttachmentBean attachmentBean:attList) {
													    //String attId = attachmentBean.getAttId();
														String attPath = attachmentBean.getPath();
														//String attMtype = attachmentBean.getMtype();
														//int attIndex = attachmentBean.getIndex();
														String attContent = attachmentBean.getContent();
														out.print("<div class=\"rh-contentPreview\"><div class=\"rh-search-file-div\">");
														//附件-图标
														out.print("<span class='blank " + attachmentBean.getIcon() + "'></span>");
									      				   //附件-标题
									      				 //  out.println("<font  color=\"#008000\">" + attTitle +"</font>");
									      				 //增加链接判断
									      				 if(attPath.startsWith("javascript:")){
									      					 out.print("<a href='javascript:void(0);' onclick=\"" + attPath + "\">" + attachmentBean.getTitle() + "</a>");
									      				  } else {
									      					   //区分移动版
				  												if(isMobileServer){
				  													attPath = attPath.replace(pcOaAddr, mobileOAaddr);
				  												} 
									      				       out.print("<a href='javascript:void(0);' onclick='javascript:openAttachment(\"" + attPath + "\",\"read." + attachmentBean.getMtype() + "\");'>" + attachmentBean.getTitle() + "</a>");
									      				  }
										      				 /**
										      				     //去除附件下载
										      				     //附件-下载
									...
									<li class="rh-res-li">
										<div class="vsc" sig="mVp" style="margin-top:15px;">
											<%
										  	flag++;
										  	}
 ...
 -->
								...
								<input type="hidden" id="rh-select-serv-id" /> <input
									...
									id="rh-select-serv-id__NAME" value='----类别选择----'
												...
												<tbody>
													<tr>
														...
														<td width="100%"><input id="cdr_min" name="beginTime"
															...
															type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="<%=SearchServ.getStartDate(filterList, outBean) %>"></td>
													</tr>
													<tr>
														<td><label for="cdr_max">到</label></td>
														<td><input id="cdr_max" type="text" name="endTime"
															style="width: 100px; height: 16px;"
															onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="<%=SearchServ.getEndDate(filterList, outBean) %>"></td>
													</tr>
													<tr>
														<td></td>
														<td class="hint"
