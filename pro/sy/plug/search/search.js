...
Search.getServ = function(event) {
	jQuery("#rh-select-serv-id").empty()
	var options = {
...
Search.clearServ = function() {
	...
	jQuery("#rh-select-serv-id__NAME").val("");
	Search.updateFilter("service");
};
...
function getServiceName(sId) {
	//var name = FireFly.getServName(sId);
	if(sId.length == 0){
		return "";
	}
...
function getSearchableServName(sId) {
	var key = "_DICT-SY_SERV_SEARCH";
	var returnValue = "";
	var dictVal = jQuery(document).data(key);
	if(dictVal == undefined){ //没有缓存数据，则重新从服务器上取的
		var res = FireFly.getDict("SY_SERV_SEARCH","","","","","");
		dictVal = res[0].CHILD;
		jQuery(document).data(key,dictVal);
	}
	jQuery.each(dictVal, function(i, n) {
		var id = n.ID;
		var name = n.NAME;	
		if (id == sId) {
			returnValue = name;
...
function parseParam(dataParams) {
    var resultData = {};
    var paStr = [];
    var str = [];
    paStr.push("{");
    if (dataParams && !jQuery.isEmptyObject(dataParams)) {
      jQuery.each(dataParams,function(i,n) {
            str.push("\"");
            str.push(i);
            str.push("\"");
          ...
          str.push("]");     
              ...
              str.push("\"");
              str.push(n.replace(/\\/g, "\\\\"));
              str.push("\"");
            }
      str.push(",");
      });
      var aa = str.join("");
      str = aa.substring(0,aa.length-1);      
    }
    paStr.push(str);
    paStr.push("}"); 
    paStr = paStr.join("");  
    paStr = paStr.replace(new RegExp("\r","gm"), "\\r").replace(new RegExp("\n","gm"), "\\n");
    return paStr;
};
