    ...
    on_presence: function (presence) {
        ...
        if (ptype === 'subscribe') {
            ...
            var contact = $('#roster-area li#' + jid_id + ' .roster-contact')
                ...
                .removeClass("offline");
            //始终以最后一次下线状态为准
            if (ptype === 'unavailable' && from == contact.attr("res")) {
                contact.addClass("offline");
            } else {
            	//put last resource 
            	contact.attr("res",from);
    ...
    on_message: function (message) {
        ...
        if (body) {
            ...
            if (0 == body.indexOf('${rh.file}') ) { 
              ...
              $('#chat-' + jid_id + ' .chat-message:last .chat-text') .append("<a href='" + fileUrl + "'>" + fileName + "</a>");
            } else {
...
//             "</span>&gt;<span class='chat-text'>" +
//             "</span></div>");
//         $('#chat-' + jid + ' .chat-message:last .chat-text') .append(body);
     ...
     * 获取聊天记录
     */
    getHisMsg: function (jid, count) {
    	Gab.connection.hismsg.listHisMsgs(jid, "", Gab.on_hisMsgs, 0);
   },
   ...
   getRcontacts: function () {
   	Gab.connection.recentcontact.listContacts("", Gab.on_Rcontacts);
  },
...
//                $(this).dialog('close');
    ...
    $('.roster-contact').live('click', function () {
        ...
        if ($('#chat-' + jid_id).length === 0) {
            ...
            //文件传送
        	var params = {};
        	params.data = "{\"SERV_ID\" : \"\", \"FILE_CAT\" : \"SHARE_FILE\"}";
        	var type = {"ID" : "default", "NAME" : "", "ID" : "SHARE_FILE", "VALUE" : 15, TYPES : "*.jpg;*.jpeg;*.png;*.gif;*.doc;*.docx;*.wps;*.xls;*.xlsx;*.ppt;*.pptx;*.txt;"};
        	var rhfile = new rh.ui.File({
        		customSettings : {						
        			progressTarget : "terMinalMindFile_fieldset"	// 显示进度的fieldset ID
        		},
        		fileType : type,
        		types : "*.jpg;*.jpeg;*.png;*.gif;*.doc;*.docx;*.wps;*.xls;*.xlsx;*.ppt;*.pptx;*.txt;",
        		id : "termianlMindFile_button",
        		postParams : params,
        		isServFile : true,
        	});
        	//send file callback
        	rhfile.setCallback(function(){
        		//after save file 
        		var fileId = rhfile.getAddFile();
        		var fileName = rhfile.getAddFileName();
        		var fileUrl = FireFly.getContextPath() + "/file/" + fileId
        		
        		//send message
                var body = "${rh.file}" + fileUrl + "?fileName=" + fileName;
        		Gab.sendMsg(jid, body);
        	...
        	$('#chat-' + jid_id).append(rhfile.obj);
           // $('#chat-' + jid_id).append("<input type='text' class='chat-input'>");
        	
        	rhfile.initUpload();
            
        	//加载聊天记录
//        	Gab.getHisMsg(jid, 10);
        	//加载最近联系人
        	Gab.getRcontacts();
...
$(document).bind('connect', function (ev, data) {
   // var conn = new Strophe.Connection( 'http://bosh.metajack.im:5280/xmpp-httpbind');
    var conn = new Strophe.Connection( 'http://192.168.1.105:7070/http-bind/');
  //  var conn = new Strophe.Connection( '/jwchat/JHB/');
