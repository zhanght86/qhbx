...
rh.ui.Text = function(options) {
	...
	this._opts = jQuery.extend(opts, options);
     //配置格式化
	this._format = this._opts.item_card_format || "";
	this._formatFun = "";
	if(this._format.length > 0){
	  this._formatFun = this._format.split(".")[1];
	}
	this._id = opts.id;
	// 是否隐藏
	...
	if (opts.isReadOnly) {// 只读
		...
		this._obj.blur(function() {
			//如果配置科学计数法，失去焦点之后就格式化，方便查看
			var _orgValue = _self.getValue();
			if (_orgValue) {
				if (_self._formatFun == "formatNum()") {
					var formatStr = _self._format.substring(0, _self._format.length - 1);
					//执行格式化
					var _formatValue = eval(formatStr + "\"" + _orgValue.replace(/\n/gi, "").replace(/\"/gi,"&quot;") + "\")");
					_self.setValue(_formatValue);
				}
			}
			// 上一个校验成功才做下一个校验
			var pass = true;
		...
		//聚焦的时间被格式化的数据就会还原，方便编辑
		this._obj.focus(function(){
		if(_self._formatFun == "formatNum()") {
		  _self._obj.val(_self.getValue())
		 }
		});
	}
	// 设置宽度
...
rh.ui.Text.prototype.fillData = function(val) {
	if (!this.isPassword) {
		//配置科学计数法，填充数据就格式化
		if (this._formatFun == "formatNum()") {
			var formatStr = this._format.substring(0, this._format.length - 1);
			val = eval(formatStr + "\"" + val.replace(/\n/gi, "").replace(/\"/gi, "&quot;") + "\")");
		}
		this._obj.val(val);
	}
};
...
rh.ui.Text.prototype.getValue = function() {
	var value = this._obj.val();
	// 有格式化就去除格式化
	if (this._formatFun == "formatNum()") {
		value = Format.clearNum(value);
	}
	return value;
};
/**
 ...
 * 给下拉框 清除 数据
 */
rh.ui.Select.prototype.removeOptionsOld = function() {
//	this._obj.find("option").remove();
	this._obj.remove();
	this._obj = jQuery("<select class='" + this._opts.cls + "' id='" + this._opts.id + "' name='" + this._opts.name + "'></select>");
	this._container.append(this._obj);
};
...
rh.ui.File = function(options) {
	...
	$strArr.push("<table class='file_uploadTable' style='border-width:0;'><tr style='border-width:0;height:auto;min-height:0px;'><td style='border-width:0;'><span class='uploadBtntr'>");
    
	
	// 按钮
	$strArr.push("<span class='uploadBtn'><a class='rh-icon rhGrid-btnBar-a' id='" + this.time  + "-upload' actcode='upload'>" +
			"<span class='rh-icon-inner' style='position:relative;'><span id='uploadHolder_" + this.time + "'></span><span class='btnName'>" + btnName + "</span></span></a></span>");
	...
	this._container.find(".uploadBtntr").first().after("<span class='modifyBtn'><a class='rh-icon rhGrid-btnBar-a' actcode='upload'>" +
			"<span class='rh-icon-inner'>修改文件信息</span></a></span>");
	...
	this.beforeEditFile = jQuery.noop// 编辑之前的操作
	this.beforeViewFile = jQuery.noop// 查看之前的操作
	this.afterModifyFile = function(datas) {}// 修改文件信息之后
	/** 队列上传完成 参数为上传文件的Bean数组 **/
	this.afterQueueComplement = jQuery.noop;
...
rh.ui.File.prototype.refresh = function(servSrcId, dataId, fileCat) {
	...
	//增加工作流显示模式参数，传递给后台
	var disMode = this.cardObj.getByIdData("WF_DISPLAY_MODE");
	var param = undefined;
	if(disMode){
		param = {"WF_DISPLAY_MODE":disMode};
	}
	
	var fileData = FireFly.getCardFile(_servSrcId, _dataId, _fileCat, param);
	this.clear();
...
/**
 * 动态设置可上传的文件类型
 * @param types 可上传类型例如：*.txt;*.doc;*.docx;
 * @param description 文件选择框下面显示的提示文字
 */
rh.ui.File.prototype.setFileTypes = function(types, description) {
	this.getUpload().setFileTypes(types, description);
}
/**
 * 获取文件上传对象
 */
...
rh.ui.File.prototype._resetAcl = function(acl){
	var aclArr = this.calcAcl(acl);
	
	// 下载权限
	var downloadAcl = aclArr[0];
	if (downloadAcl) {
		if (Browser.ignoreButton({"ACT_MOBILE_FLAG":"2"})) { // 移动平台禁用下载按钮
	...
	if (modifyAcl) {
		this._container.find(".modifyBtn").show();
	} else {
		this._container.find(".modifyBtn").hide();
	}
	// 删除权限
	...
	var uploadAcl = aclArr[3];
	if (uploadAcl) {
		if (Browser.ignoreButton({"ACT_MOBILE_FLAG":"2"})) { // 移动平台禁用上传按钮
			this._container.find(".uploadBtn").hide();
			this._container.find(".uploadBtntr").hide();
		} else {
...
rh.ui.File.prototype.initUpload = function() {
	...
	if (!this._upload) {
		...
		settings.swfupload_loaded_handler = function (){
			...
			_self.afterFlashLoaded.call(_self);
		};
 ...
 * flash加载完成之后
 */
rh.ui.File.prototype.afterFlashLoaded = function() {
};
/**
 * 显示进度条
 */
...
rh.ui.File.prototype.fillData = function(fileData){	
	...
	if  (len > 0) {
		...
		for (var i = 0; i < len; i++) {
			...
			$strArr.push("<span class='fileName' title='" + titleName + "'>");
...
//			$strArr.push("</a>");
			...
			$strArr.push("<a class='download_file' target='_blank' href='javascript:;'>下载</a>");
			$strArr.push("</span>");
			$strArr.push("</span>");
			$strArr.push("</td></tr></table>");
			...
			$edit.data("id", fileId).data("name", name).data("wfNIId", wfNIId).click(function(){
				// 没有保存不允许编辑
				//特殊服务允许不保存的情况下编辑
				var cs = System.getVar("@C_BN_FILE_EDIT@")||"";
				var c = cs.split(",");
				var cFlag = false;
				for(var i=0;i<c.length;i++){
					if(_self.cardObj && c[i]==_self.cardObj.servId){
						cFlag=true;
					}
				}
				if(cFlag){
			...
			// 注册查看事件
			$view.data("fileBean",file).data("id", fileId).data("name", name).click(function(){
				var $this = jQuery(this);
...
//			});
			...
			// 注册下载事件
			$download.attr("href", "/file/" + fileId);
...
//			});
			...
			if (!editAcl || !Browser.supportNTKO() || !RHFile.isWorkDoc(fileId)) { // 没有权限或者不支持NTKO或者不是办公文档
				$edit.parent().hide();
			}
			if (!viewAcl) {
				$view.parent().hide();
...
rh.ui.File.prototype.modifyFile = function() {
	...
	if (idArr.length > 0) {
	    ...
	    var temp = {"sId":"SY_MODIFY_FILE","pCon":div,"batchFlag":true,"showTitleBarFlag":"false",
				"selectView":false,"pkHide":true,"replaceQueryModel":1,"parHandler":_self.cardObj,"resetHeiWid":function(){},"extWhere":extWhere};
		var listView = new rh.vi.listView(temp);
		listView.show();
		
		listView.afterSave = function(datas) {
			_self.refresh();
			_self.afterModifyFile.call(_self, datas);
		};
	} else {
		alert("没有文件需要修改！");
	}
...
//	}
	...
	var _officeParams = {"revision":revision};
	if(this.replaceBookmark){
		_officeParams["bookmark"] = this.replaceBookmark;
	}
...
rh.ui.Textarea = function(options) {
	...
	this._obj = this.obj = jQuery($strArr.join(""));
	this._container = jQuery("<div class='blank' style='height:"+this._height+"px;'></div>").append(this._obj);
	if (opts.isReadOnly) {// 只读
		this.disabled();
	} else {
		if (opts.isNotNull) {
...
rh.ui.BigText.prototype.createEditor = function() {
	...
	var toolbars = [
	                ['fullscreen', 'source', '|', 'undo', 'redo', '|',
	                  'bold', 'italic', 'underline', 'strikethrough', 'superscript', 'subscript', 'autotypeset','|', 'forecolor', 'backcolor', 'insertorderedlist', 'insertunorderedlist', 'cleardoc', '|',
	...
	var config = {
	    ...
	    ,fontfamily:[
			...
			{ label:'仿宋',name:'fangsong',val:'仿宋,FangSong,sans-serif'},
			{ label:'',name:'kaiti',val:'楷体,楷体_GB2312, SimKai'},
			{ label:'',name:'yahei',val:'微软雅黑,Microsoft YaHei'},
			{ label:'',name:'heiti',val:'黑体, SimHei'},
			{ label:'',name:'lishu',val:'隶书, SimLi'},
			{ label:'',name:'andaleMono',val:'andale mono'},
			{ label:'',name:'arial',val:'arial, helvetica'},
...
rh.ui.linkSelect.prototype.fillData = function() {
	...
	if (this.obj.find(".rh-linkselect-table")) {
		this.obj.find(".rh-linkselect-table").remove();
	}
	this.table = jQuery("<table class='rh-linkselect-table' style='min-width:475px;max-width:485px;'></table>");
//	if (data.length == 0) {
...
//	}
	jQuery.each(data,function(i,n) {
		var tr = jQuery("<tr></tr>");
		var td1 = jQuery("<td class='ui-linkSelect-default-td cp' style='border:0px !important;'><a>" +(i+1)+". "+n.TITLE + "</a></td>").appendTo(tr);
		var td2 = jQuery("<td class='ui-linkSelect-default-td '  style='border:0px !important;'>" + n.S_USER__NAME + "</td>").appendTo(tr);
		var td3 = jQuery("<td class='ui-linkSelect-default-td ui-linkselect-default-delete'  style='border:0px !important;min-width:30px !important;'><a href='javascript:void(0);'>删除</a></td>").appendTo(tr);
		//相关文件的id 和服务 隐藏
		var td4 = jQuery("<td class='dataPk' style='display:none'>"+n.RELATE_DATA_ID+"</td>").appendTo(tr);
		var td5 = jQuery("<td class='dataTitle' style='display:none'>"+n.TITLE+"</td>").appendTo(tr);
		// 输入设置
		var array = eval(_self._configs);
		var queryServ = n.RELATE_SERV_ID;
		var editServ = "";
		if (array != null && array.length > 0) {
			// 是否已指定服务编辑关联的数据
			if (array[0].EDIT_SERV_ID && array[0].EDIT_SERV_ID != '') {
				editServ = array[0].EDIT_SERV_ID;
			}
			// 是否已指定服务查看关联的数据
			if (array[0].QUERY_SERV_ID && array[0].QUERY_SERV_ID != '') {
				queryServ = array[0].QUERY_SERV_ID;
			}
		}
		td1.bind("click",function(){//debugger;
			var opts = {"tTitle":n.TITLE,"url":queryServ+".card.do?pkCode="+n.RELATE_DATA_ID,"menuFlag":3};
			Tab.open(opts);
		});
		if (editServ != '') {
			//获取流程节点变量canEditRelateFile
			if (_self.cardObj && _self.cardObj.wfCard) {
				var canEditRelateFile = _self.cardObj.wfCard.getCustomVarContent("canEditRelateFile");
				if (canEditRelateFile && "true" == canEditRelateFile) {
					// 编辑
//					var td4 = jQuery("<td class='ui-linkSelect-default-td ui-linkselect-default-edit' align='right'  style='border:0px !important;min-width:30px !important;'><a href='javascript:void(0);'>编辑</a></td>").appendTo(tr);
//					td4.bind("click",function(){
//						var opts = {"tTitle":n.TITLE,"url":editServ+".card.do?pkCode="+n.RELATE_DATA_ID,"menuFlag":3};
//						Tab.open(opts);
//					});
				}
			}
		...
		td3.unbind("click").bind("click",function(event) {//删除绑定
    ...
    if (this._isReadOnly === true) {
    	...
    	this.table.find(".ui-linkselect-default-edit").hide();
    }
	//添加外边框，2013-9-10 09:13:20 hdy
	this.tableParentDiv = jQuery("<div style='float:left;width:100%;'></div>");
	this.table.appendTo(this.tableParentDiv);
	this.tableParentDiv.appendTo(this.obj);
	if ((this.table.html() || "") == "") {
		this.table.html("<tr><td>暂无相关文件！</td></tr>");
	}
};
/**
 * 增加添加后方法
...
rh.ui.linkSelect.prototype.disabled = function() {
	this.obj.addClass("disabled");
    this.obj.find(".ui-linkselect-default-delete").hide();
    //只隐藏相关文件按钮
    this.obj.find("a").eq(0).hide();
};
/**
 * 取消只读
 */
rh.ui.linkSelect.prototype.enabled = function() {
	this.obj.removeClass("disabled");
    this.obj.find(".ui-linkselect-default-delete").show();
    this.obj.show();
};
/**
 * 隐藏
 ...
 * 获取父容器
 */
rh.ui.linkSelect.prototype.getContainer = function(){
 ...
 * 获取标签对象
 */
rh.ui.linkSelect.prototype.getLabel = function(){
	return this.getContainer().parent().parent().find("#" + this.opts.id + "_label");
};
...
rh.ui.Date = function(options) {
	...
	if (Browser.isMobileOS()) {
		...
		var type = configArray[0];
		this.type = type;
		this.html5DP = false;
		if (Browser.supportHTML5()) {
			this.html5DP = true;
		}
		this.mobisrollDP = !this.html5DP;
		if (this.html5DP) {
			if (type == "DATE") { //yyyy-mm-dd
				type = "date"; //yyyy-mm-dd
			} else if (type == "DATETIME") { //yyyy-mm-dd HH:MM:ss
	    		type = "datetime-local"; //
		    } else if(type == "DATETIMEH") { //yyyy-mm-dd HH
		    	type = "datetime-local"; //
		    } else if(type == "DATETIMEHM") { //yyyy-mm-dd HH
		    	type = "datetime-local";
			...
			this._obj = this.obj = jQuery("<input type='" + type + "' class='" + opts.cls + "' id='" + opts.id + "' />");
			this._obj.css("width", "100%");
		} else if (this.mobisrollDP) {
			var opt = {
				"preset": "datetime",
...
rh.ui.Date.prototype.fillData = function(val) {
	if (Browser.isMobileOS() && this.html5DP) {
		if (val && val.length > 0) {			
			var year,month,date,hours,minute,second;
			var dLength = val.length;
			if (this.type == "DATE") { //yyyy-mm-dd
				if (dLength > 10) {
	    			val = val.substr(0, 10);
	    		}
			} else if (this.type == "DATETIME") { //yyyy-mm-dd HH:MM:ss
	    		if (dLength == 16) {
	    			val = val + ":00"
	    		} else if (dLength > 19) {
	    			val = val.substr(0, 19);
				...
				month = parseInt(val.substr(5, 2) || "01")-1;
				date = parseInt(val.substr(8, 2) || "01");
				hours = parseInt(val.substr(11, 2) || "00");
				minute = parseInt(val.substr(14, 2) || "00");
				second = parseInt(val.substr(17, 2) || "00");
				val = new Date(year,month,date,hours,minute,second).format("isoDateTime");				
		    } else if (this.type == "DATETIMEH") { //yyyy-mm-dd HH
		    	if (dLength > 13) {
	    			val = val.substr(0, 13);
	    		}
		    	year = parseInt(val.substr(0, 4));
				...
				second = parseInt(val.substr(17, 2) || "00");
				val = new Date(year,month,date,hours,minute,second).format("isoDateTime");	
		    } else if (this.type == "DATETIMEHM") { //yyyy-mm-dd HH:MM
		    	if (dLength > 16) {
	    			val = val.substr(0, 16);
	    		}
		    	year = parseInt(val.substr(0, 4));
				...
				second = parseInt(val.substr(17, 2) || "00");
				val = new Date(year,month,date,hours,minute,second).format("isoDateTime");
		    } else if (this.type == "YEAR") { //yyyy
		    	if (dLength > 4) {
	    			val = val.substr(0, 4);
	    		}
		    } else if (this.type == "MONTH") { //yyyy-mm
		    	if (dLength > 7) {
	    			val = val.substr(0, 7);
	    		}
 ...
 * 重新设置时间
 */
rh.ui.Date.prototype.setValue = function(val) {//YYYY-MM-dd HH:MM:ss:sss
	this.fillData(val);
	this.onchange();
};
 ...
 */
rh.ui.Date.prototype.getValue = function(beforeChangedVal) {
	...
	if (Browser.isMobileOS() && this.html5DP) {
		...
		if (val && val.length > 0) {
			...
			//2.修正到期望的格式
			var SLENGTH = beforeChangedVal ? beforeChangedVal.length : 0;
			var dLength = val.length;
			if (this.type == "DATE") { //yyyy-mm-dd
				if (dLength > 10) {
	    			val = val.substr(0, 10);
			...
			if (SLENGTH > 16) {
				if (dLength > 16) {
					if (beforeChangedVal.substr(0,16) == val.substr(0,16)) {
						return beforeChangedVal;
					}
				} else {
					if (beforeChangedVal.substr(0,dLength) == val) {
						return beforeChangedVal;
					}
				}
			} else {
...
rh.ui.Date.prototype.enabled = function() {
	var _self = this;
	var date = _self._obj;
	if (!Browser.isMobileOS() || Browser.supportHTML5()) {
...
rh.ui.DictChoose.prototype._getConfigVal = function(){
	var _self = this;
	_self.beforeGetConfigVal(); //执行自定义获取输入框内容之前操作
 ...
 * @return 取得服务定义字段定义中配置的值
 */
rh.ui.DictChoose.prototype.beforeGetConfigVal = function(){
	//var _self = this;
 ...
 * @param description 文件选择框下面显示的提示文字
 */
rh.ui.Upload.prototype.setFileTypes = function(types, description) {
	this._swfObj.setFileTypes(types, description);
};
/**
 * 销毁
 */
...
rh.ui.Form.prototype._resetPosition = function() {
	if (!jQuery.isEmptyObject(this._resetPositionOpts)) {
		...
		for (var itemCode in this._resetPositionOpts) {
			...
			blank.parent().width(width);
			right.parent().width(blank.width());
				...
				switch (type) {
					case UIConst.FITEM_ELEMENT_INPUT :
						ui = new rh.ui.Text({
							...
							item_card_format : data.ITEM_CARD_FORMAT,
							regular : regular,
							hint : hint,
					...
					case UIConst.FITEM_ELEMENT_TEXTAREA :
						ui = new rh.ui.Textarea({
							...
							_default : data.ITEM_INPUT_DEFAULT,
							width : data.ITEM_CARD_WIDTH,
							height : StrToJson(data.ITEM_INPUT_CONFIG)==null?"":StrToJson(data.ITEM_INPUT_CONFIG).height,
							isNotNull : isNotNull,
							isReadOnly : isReadOnly,
					...
					case UIConst.FITEM_ELEMENT_DATA_SERVICE :
						...
						var confStr = data.ITEM_INPUT_CONFIG;
						var confArray = confStr.split(",");
					    var sId = confArray[0];
...
rh.ui.Form.prototype.fillData = function(data) {
	...
	for (var key in this._items) {
		...
		if (item.type == "File") {
			var fileItemList = this.cardObj.getByIdData("_FILE_ITEM_LIST_");
			var fileData = null;
			if (fileItemList) { //如果卡片页面已经存在附件列表数据，则从页面中获取数据，否则从服务器装载
				fileData = fileItemList[key];
			} else {
				fileData = this._loadFile(key);
			}
...
rh.ui.Form.prototype._loadFile = function(key){
	// 载入附件
	...
	var fileData = FireFly.getCardFile(this._servSrcId, servDataId, key, param);
	return fileData;
}
...
rh.ui.Form.prototype.getChangedItems = function() {
	...
	for (var key in items) {//遍历所有表单域
		...
		var curVal = item.getValue(theVal);// 当前值
		if (theVal == undefined) {
			if (curVal) {
