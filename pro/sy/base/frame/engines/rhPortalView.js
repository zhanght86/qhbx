...
rh.vi.portalView = function(options) {
	...
	this._expParam = "";
	//模版缓存启用后，根据url值进行缓存
	this.onlyUrl = "SY_COMM_TEMPL.getPortal.do?pk=" + this._pkCode  
...
rh.vi.portalView.prototype._bldParamContent = function(paramObj) {
	...
	//截取组件主键中包含^^字符
	if(pcId.indexOf("^^") > 0){
	   var pcArr = pcId.split("^^");
	   pcpageId =pcArr[0];
	} else {
	   pcpageId = pcId;
	}
	var comObj = FireFly.byId("SY_COMM_TEMPL_COMS",pcpageId);
...
rh.vi.portalView.prototype._addToModel = function(id) {
	//随机生成模版id，用于组件的重复添加
	var newId = id + "^^" + StringUtils.randomNum();	
	var extendCon = jQuery("<div></div>").addClass("portal-temp").attr("comid",newId);
...
//		  var tarHei = Math.floor(jQuery(this).height()/2);
//		  if ((uiTop + tarHei) < 0) {
