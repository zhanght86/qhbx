...
rh.vi.pageView = function(options) {
	...
	this.toDoAllCount = 0;
	this._openTabParams = null; //打开tab扩展参数
};
/*
 * 渲染主方法
 */
...
rh.vi.pageView.prototype._bldBanner = function() {
	...
	if (comJsonStr.length > 0) {
		...
		jQuery.each(cmJson,function(i,n) {
			...
			var container = jQuery("<div class='rh-head-cm-container'></div>").appendTo(cmLinkCon);
...
//				}
	...
	if (typeof bannerSearch == "string" && (bannerSearch !== "false")) {
		...
		var temp = jQuery("<input type='text' class='rh-head-sea-input' style='margin-top:13px;' value='智能搜索'/>").appendTo(seaCon);
		...
		var seaIcon = jQuery("<div class='rh-head-sea-icon' style='margin-top:13px;'></div>").appendTo(seaCon);
	...
	//个人、兼岗、委托区
	var perCon = jQuery("<div style='margin-top:13px;background:none;'></div>").addClass("rh-head-per");
	...
	var perConIn = jQuery("<div></div>").addClass("user_info").attr("title", System.getUser("ODEPT_NAME")).appendTo(perCon);
	var userSex = System.getVar("@USER_SEX@");
	var perImg = System.getUser("USER_IMG");
	//var perDiv = jQuery("<img class='rh-head-per-icon'></img>").attr("src",perImg).appendTo(perConIn);
	var perTip = "";
	//var perTip = jQuery("<div class='user_info'><span>" + System.getUser("DEPT_NAME")
	...
	var serverClusterNodeInfoCon = jQuery("<div style='position:absolute;top:4px;right:490px;'></div>").appendTo(jQuery("#banner"));
