...
rh.vi.cardView.prototype.show = function() {
	...
	this._bldCardLayout();
	this._afterLoad();
	this.callMergerFjAndXgwj(); // 合并附件相关文件字段
	this.removeZhegnWenUser(); //去除正文上传人姓名显示
};
/*
 * 布局卡片tabs和主功能页
 */
 ...
 * @param {Object} origPK
 */
rh.vi.cardView.prototype.setOrigPK = function(origPK) {
	this.byIdData[this.servKeys] = origPK;
};
/*
 * 仅重新读取form数据并填充
 */
...
rh.vi.cardView.prototype._resetHeiWid = function(bottomHei) {
    ...
    jQuery(document).ready(function() {
    	...
    	if (bottomHei) {//有高度差时
    		tempH = _self.formCon.height() + bottomHei + 85; //form+按钮条+标签条
    	}
 ...
 * 设置子tab的只读
 */
rh.vi.cardView.prototype.tabReadOnly = function(sid) {
    this.tabReadOnlyArray = {};
    this.tabReadOnlyArray[sid + "_READONLY"] = true;
};
/*
 * 设置子tab的显示
 */
 ...
 */
rh.vi.cardView.prototype.setParams = function(addParams) {
	for (i in addParams) {
		this.params[i] = addParams[i];
	}
};
/*
 * 设置点击返回时刷新父列表页面
...
rh.vi.cardView.prototype.mergerFujianAndXgwj = function(mergerObj, fujianItem, xgwjItem){
	...
	fjAxgwj.obj.parent().css({"min-height":"27px"});
};
 ...
 * 去除正文中起草人
 */
rh.vi.cardView.prototype.removeZhegnWenUser = function(){
	var _self = this;
	var zhengWenItem = _self.getItem("ZHENGWEN"); //获取正文字段
	if (!zhengWenItem) {
		return;
	}
	var uploadTable = zhengWenItem.obj.find("table[class='file_uploadTable']");
	uploadTable.find("td").each(function(){
		var _self = this;
		var uploadUser = jQuery(_self).find("span[title='上传人']");
		uploadUser.remove();
	});
};
