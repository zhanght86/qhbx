...
rh.vi.listView = function(options) {
	...
	this._cardRead = true;
	this._byIdFlag = false;//是否可以进入卡片查看
    ...
    //jQuery(document).ready(function(){
    	//setTimeout(function() {
		    ...
		    jQuery.each(itemArray,function(i,n) {
				...
				//如果有传递pid参数，则替换默认配置
				if (_self.slimParams()["DICT_PID"]) {
					pId = _self.slimParams()["DICT_PID"];
				}
                var expandLevelParam = confJson && (confJson.expandLevel) ? confJson.expandLevel:1;
				var setting = {
						rhexpand: false,
...
rh.vi.listView.prototype.refreshGridBodyNoResize = function(options) {
	...
	if (this.opts._HIDE_ && options) {//需隐藏的列
		options["_HIDE_"] = this.opts._HIDE_;
	}
