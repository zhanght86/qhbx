	...
	show:function(msg, parentFlag, scopeObj) {
		...
		if ((typeof parentFlag == "boolean") && parentFlag) {
			...
			if (scopeObj) {
				...
				//如果提交按钮是在意见填写界面点击，则将提示信息展示在最外边的容器中，避免当前卡片页面关闭后，无法看到提示信息。
				if (jQuery("#mindContainerHI")) {
					tip.appendTo(parent.parent.jQuery(".tabUL"));
				} else {
...
var Tools = {
	...
	systemVarReplace : function(str) {
		str = str.replace(/@USER_CODE@/g, System.getVar("@USER_CODE@"))
				...
				.replace(/@USER_WORK_NUM@/g, System.getVar("@USER_WORK_NUM@"));
		return str;
	},
	/*父层级级变量替换*/
...
var RHFile = {
  ...
  //TODO: url = FireFly.getContextPath() + url 
	...
	readFile : function (url,fileName){
		if (Browser.supportNTKO() && this.isWorkDoc(fileName)) { // 支持NTKO且是办公文档
			zotnClientNTKO.DownloadFile(url, fileName, false, true, false, true);
		} else {
...
var Select = {
	...
	usualContent : function (opts, viewer,positionArray,dialogSizeArray) {
		opts.optObj.bind("click", function(event) { // 为对象添加一个单击事件
			var configStr = "SY_COMM_USUAL,{'SOURCE':'USUAL_ID~USUAL_TYPE~TITLE',"
					+ "'HIDE':'USUAL_ID',"
