...
_viewer.beforeSave = function() {
	var outBean = FireFly.doAct("UM_WEBSERVICE_SERV", "getUmUser", {"USER_WORK_NUM":_viewer.getItem("").getValue()});
	if (outBean["_MSG_"].indexOf("ERROR,") >= 0) { //返回错误
		_viewer.cardBarTipError(outBean["_MSG_"].replace("ERROR,", ""));
		return false;
	} else if (outBean["_MSG_"].indexOf("OK,") >= 0) { //返回成功
		return true;
	}
};
