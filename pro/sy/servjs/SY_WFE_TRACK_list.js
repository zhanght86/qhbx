...
_viewer.grid.getCheckBox().each(function(i,item){
	...
	var nodeDays = _viewer.grid.getRowItemValueByElement(chkboxObj,"NODE_DAYS");
	jQuery(jQuery("td[icode='NODE_DAYS']")[i]).text(parseInt(nodeDays/60/24)+"天/"+parseInt(nodeDays/60%24)+"小时/"+parseInt(nodeDays%60)+"分");
});
//隐藏已经办结的记录对应的radio
...
figureBtn.bind("click",function() {
	...
	var opts = {"tTitle":"图形化流程跟踪",
			"url":"SY_WFE_TRACK_FIGURE.show.do?data=" + encodeURIComponent(JsonToStr(_viewer.params)),
...
if (_viewer.getBtn("cuiban")) {
	_viewer.getBtn('cuiban').unbind("click").bind("click",function() {
		...
		//节点实例ID
		params.NI_ID = _viewer.grid.getSelectItemVal("NI_ID");
		FireFly.doAct("SY_WFE_REMIND", "remindByTodo", params, false, false, function(result){
			alert(result._MSG_);
		});
