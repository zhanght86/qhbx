...
<!---------------返回顶部的Js/CSS------结束--------------->
		...
		<script type="text/javascript" src="/sy/comm/zhidao/js/zhidao_specialist.js"></script>
		<script type="text/javascript">
			var tempId = '${userZhidao_center_tmpl_id}';
		</script>
	...
	<body class="layout-center">
		<div class='layout-center-div' style="background:none;background-color:#FFF!important;">
		<div class="line">
			<!-- userbar -->
		</div>
<!--------------search---------------------->				
		...
		.banner-item-arrow{float:left;height:40px;width:40px;margin:1px 11px;opacity:.3;filter:alpha(opacity=30);cursor:pointer;position:relative;}
		.banner-item:hover{border:1px solid #5d5d5d;opacity:1;filter:alpha(opacity=100);}
		.banner-item-arrow:hover{opacity:1;filter:alpha(opacity=100);}
		.banner-tab .selected{border:1px solid #444!important;margin:0 10px;opacity:1;filter:alpha(opacity=100);}
		.banner-item img{height:40px;width:auto;display:inline-block;vertical-align: middle;}
					...
					<div href="javascript:void(0);" target="_self" class="banner-img <%=(i==0) ? 'show' : ''%>" style="display:none;width:<%=width%>px;"> 
						<span style="background: url(<%=list[i].img%>) center center no-repeat;">
							<div class="center-div">
								<div style="width:103px;height:103px;position:absolute;top:28px;margin-left:27px;text-align:center;overflow:hidden;background-color:white;">
									<img style="width:auto;height:103px;display:inline-block;vertical-align:middle;" src="<%=list[i].item%>?size=103x103" />
								</div>
								<div class="card">
									<div class="card-left">
									...
									%>
										<div style="height:33px;">
											...
											<%
												if(activity_index == 0){
											%>
												<p class="css-activity-img" style="background-position:0px 0px;"></p>
											<%
												}else if(activity_index == 1 || activity_index == 2){
											%>
												<p class="css-activity-img" style="background-position:0px -50px;"></p>
											<%
												}else{
											%>
												<p class="css-activity-img" style="background-position:0px -100px;"></p>
											<%
												}
											%>
											<p class="css-activity-font" style="width:100px;"><%=activity_item.ACT_CODE__NAME%></p>
											<p class="css-activity-disName" onclick="linkActivity('<%=activity_item.ACT_CODE%>','<%=activity_item.DATA_ID%>','<%=activity_item.DATA_DIS_NAME%>','<%=activity_item.Q_ID%>','<%=activity_item.A_ID%>');"><%=activity_item.DATA_DIS_NAME%></p>
											<p class="css-activity-font" style="width:60px;"><%=activity_item.S_ATIME%></p>
										</div>
			...
			<div id="banner-tab" class="banner-tab"> 
				<div class="banner-tab-wrap" style="top:70px;"> 
					...
					<div class="banner-tab-inner" style="width:<%=width%>px"> 
						...
						<span id="prev" class="banner-item-arrow" style="width:14px; height:40px; background:url(/sy/comm/zhidao/baidu_style_files/thin_left_right_arrow.gif);background-position:-17px 0px;">
						</span>
						...
						<%for (var i = 0; i < list.length; i++) {%>
							<%if (T.browser.ie && T.browser.ie < 7) {%> 
								<a href="javascript:void(0)" onFocus="this.blur()" class="aaa banner-item <%if (i == 0) {%>selected<%}%> <%if (i>=8){%>img_hide<%}else{%>img_show<%}%>">
									<img src="<%=list[i].item%>?size=40x40" content="<%=list[i].content%>" userName="<%=list[i].userName%>" deptName="<%=list[i].deptName%>" question="<%=list[i].question%>" answer="<%=list[i].answer%>" >
								</a> 
							<%} else {%> 
						...
						<span id="next" class="banner-item-arrow" style="width:14px; height:40px; background:url(/sy/comm/zhidao/baidu_style_files/thin_left_right_arrow.gif);background-position:0px 0px;">
						</span>
		...
		var canshuInit = function(){
			...
			jQuery.each(specialistData,function(i,obj){
				...
				var userImgSrc = obj.USER_ID__IMG;
				if (userImgSrc != undefined && userImgSrc != '') {
					userImgSrc = userImgSrc.split("?")[0];
				} else {
					userImgSrc = '/sy/theme/default/images/common/rh-lady-icon.png';
				}
				temp["item"] = userImgSrc;
				temp["url"] = "index.html";
				...
				jQuery(activityList).each(function(j,item){
					...
					activityList["ACT_CODE"] = item.ACT_CODE;
					activityList["DATA_ID"] = item.DATA_ID;
					activityList["ACT_CODE__NAME"] = item.ACT_CODE__NAME;
					activityList["DATA_DIS_NAME"] = Format.limit(23,item.DATA_DIS_NAME);
					activityList["S_ATIME"] = jQuery.timeago(item.S_ATIME.substring(0,19));
					if(item.answer != '' && item.answer != undefined){
						activityList["Q_ID"] = item.answer.Q_ID;
						activityList["A_ID"] = item.answer.A_ID;
					}else{
						activityList["Q_ID"] = "";
						activityList["A_ID"] = "";
					}
			...
			if(temp_banner && temp_banner.length <= 3){
				temp_banner.push(1);
			}
		}
		        ...
		        if(answer.length > 0 && question.length > 0){
					...
					jQuery("user_content").html(
						'<dl id="aq">'+
							...
							'<dd class="info-user">回复者：'+userName+'</dd>'+
						'</dl>'
					);
		        }
		...
		<script>
			jQuery(document).ready(function(){
				//jQuery("#banner").append("<div class='div_1' style='width:200px; height:auto;left:300px;top:30px;'><div class='prof-thanks-ctn'><div class='prof-thanks-info'><p>香*****：</p><p class='prof-thanks-txt'>谢谢汤律师耐心为我答疑，当初我只是抱了尝试心态问了一下，没想到你居然回复的那么仔细！</p><p class='prof-thanks-pro'><strong id='userName'>张金伟</strong><span id='deptName'>项目中心</span></p></div><img id='user_img' src='/sy/theme/default/images/common/user0.png' alt=''></div></div>");
	...
	<div id="body" class="container" style="width:1160px;margin:0 20px;overflow:hidden;">
		          ...
		          <div class="info-left-leftside">
		           ...
		           <#list tag_list as channel>
		              <dl alog-group="hm-cate" data-cid="${channel_index + 100}">
		                <dt> 
		                	<a class="js-spec-chnl" dataId="${channel.CHNL_ID}" href="javascript:void(0)" onclick="javascript:renderSpecByChnl(1,'${channel.CHNL_ID}','${channel.CHNL_NAME}')"> 
		                ...
		                <dd>
		                  <@channel_list debugName="${channel.CHNL_NAME}子分类" channelId="${channel.CHNL_ID}"> 
		                  	<#list tag_list as sub>
		                        	<a class="js-spec-chnl" dataId="${channel.CHNL_ID}" href="javascript:void(0)" onclick="javascript:renderSpecByChnl(1,'${sub.CHNL_ID}','${sub.CHNL_NAME}')"> 
...
<!--------------------------------------全部专家--------------------------->        
	<div class="expert-bottom-right w800" style="width:680px;">
	    <div class="expert-category-user">
	        <h4 id="expertAll" style="width:640px;">
	            全部专家
	            <span>
	                (3989)
	            </span>
	        </h4>
	        <ul id="expertList" style="width:640px;">
		...
		<script type="text/javascript">
				...
				function speciaFun(curPage,chnl_id,chnl_name){
					...
					if(allnum>0){//如果有数据
						//清空原来的数据
						jQuery("#expertList").html("");
						//滚动条滚上去
						parent.jQuery("html,body").animate({scrollTop:0},500);
						var specialistData = specialPages._DATA_;
						//console.debug(specialistData);
						jQuery.each(specialistData,function(i,obj){
							//console.debug(obj);
							var spec_type = obj.q_and_a.SPEC_TYPE;
							var specTypeStr = "";
							if(spec_type == 1){
								//specTypeStr = "<div class='leader-logo' style='height:41px;'></div>";
							}else if(spec_type == 2){
								//specTypeStr = "<div class='dept-logo' style='height:41px;'></div>";
							}else if(spec_type == 3){
								//specTypeStr = "<div class='busi-logo' style='height:41px;'></div>";
							}
							...
							jQuery("<li style='padding-top: 0px;'>"+
									"<div>"+
										"<span style='width:auto;'>"+
											specTypeStr+
											"<a style='width:auto;' href='javascript:othersZhidao(&apos;${userZhidao_center_tmpl_id}&apos;,&apos;"+obj.USER_ID+"&apos;)'>"+
												formatUserImg(obj.USER_ID,obj.USER_ID__IMG,'','','80px','80px','')+
											"</a>"+
										"</span>"+
					...
					var nextPage = ""; // 下一页
					if (curPage > 1) {
						prePage = '<span onclick="goPage(\''+(parseInt(curPage)-1)+'\',\''+chnl_id+'\',\''+chnl_name+'\');" class="spec-page-span">上一页</span>';
					}
					if (curPage < speciaPageNum) {
						nextPage = '<span onclick="goPage(\''+(parseInt(curPage)+1)+'\',\''+chnl_id+'\',\''+chnl_name+'\');" class="spec-page-span">下一页</span>';
					}
					jQuery("#specia_page").append(
						'<div class="ui-pager pager-center">'+
			            	'<div id="js-more-a" class="btn-loading btn-load-more" href="javascript:void(0);" style="width:662px;background:-moz-linear-gradient(center top , #F8F8F9, #E6E6E8) repeat-x scroll 0 0 #F1F1F2;">'+
			            		prePage +
			            		nextPage +
			            	'</div>'+
			        	'</div>'
				...
				function goPage(page,chnl_id,chnl_name){
					//jQuery("#expertList").empty();
					jQuery("#specia_page").empty();
					speciaFun(page,chnl_id,chnl_name);
				}
				//动态加载更多
				$(window).scroll(function(){
			        ...
			        if(a == b) {
			            setTimeout(function(){
			            	jQuery("#js-more-a").trigger("click");
			            },1500);
			        }
			    });
			</script>
	...
	<div style="width:280px; height:auto; float:left; margin-left:20px;">
		...
		<!------------分类专家排行-----开始-------------->
		  <!---------------把总积分排行移到这里----开始------------->
			<div class="mod rank tabs ui-tabs-aside user rank3" style="padding:0px;">
					<div class="hd topper">
						<ul class="out-ul">
							<li class="li-div" style="width:75px;">
								<span class="li-span">专家排行</span>
							</li>
							<li class="li-gap"></li>
							<li class="li-div show li-selected" num="1" style="width:66px;">
								<a>
									<span class="li-span">总积分</span>
								</a>
							</li>
							<li class="li-gap"></li>
							<li class="li-div show" num="2" style="width:66px;">
								<a>
									<span class="li-span">本月</span>
								</a>
							</li>
							<li class="li-gap"></li>
							<li class="li-div show" num="3" style="width:66px;">
								<a>
