...
var fcViews = fc.views = {"month":MonthView,"basicWeek":BasicWeekView, "basicDay":BasicDayView, "agendaWeek":AgendaWeekView, "agendaDay":AgendaDayView};
...
function Calendar(element, options, eventSources) {
    ...
    function initialRender() {
        ...
        element.addClass('fc');
//alert(options.isRTL ? options.theme ? "THEME" : "RTL" : "UNKNOW");
        if (options.isRTL) {
        ...
        content = $("<div class='fc-content " + tm + "-widget-content' style='position:relative;'/>").prependTo(element);
        if (enableHeader) {
    ...
    function changeView(newViewName) {
        if (!currentView || newViewName != currentView.name) {
            ...
            if (currentView) {
                ...
                newViewElement.appendTo(content);
                currentView = viewInstances[newViewName] = new fcViews[newViewName](newViewElement, t);
            }
            if (enableHeader) {
                if (oldView) {
    ...
    function getViewElem(){
        var rtnArray = [];
        for(var s in fcViews){
            if (options["buttonText"][s]) {
                rtnArray.push({"key":s,"value":options["buttonText"][s]});
            }
        }
        return rtnArray;
    }
...
function Header(calendar, options) {
    ...
    function updateState() {
        var currentView = calendar.getView();
...
//      alert(sources[1][0].title + ":" + sources[1][1].title);
        for (var i=0; i<len; i++) {
...
//alert(events[i].title  + ":" + source[i].title);
...
//alert(head.parent().html());
            // dateTime segment body
            ///构建时间分片Begin
            d = zeroDate();
            d = addMinutes(d, minMinute);
            var maxd = addMinutes(cloneDate(d), maxMinute);
            s = "<table>";
            //依据slotMinutes配置进行时间段分割
            for (i=0; d < maxd; i++) {
            ...
            bodyTable.appendTo(bodyContent);
...
//alert("skeleton already built, just modify it");
            // skeleton already built, just modify it
            clearEvents();
            // redo column header text and class
    ...
    function setWidth(width) {
        ...
        bodyTable.width('');
        var topTDs = head.find('tr:first th'),allDayLastTH = head.find('tr.fc-all-day th:last'),stripeTDs = bg.find('td'),clientWidth = body[0].clientWidth;
        //TODO 这块使用2次clientWidth解决IE6问题
    ...
    function resetScroll() {
        ...
        //alert(opt('firstHour'));
        //定位到8点
        var user = FireFly.getCurUser();
        var startdate = user.limitAmDateLine;
        if (startdate = "") {
          scrollDate.setHours(9);
        } else {
            var s = startdate.split(":");
            if (s[0] != "") {
                scrollDate.setHours(s[0]);
            } else {
                scrollDate.setHours(9);
            }
            if (s.length > 1 && s[1] != "") {
                scrollDate.Minutes(s[1]);
            }
        }
    ...
    function slotBind(tds) {
        tds.click(slotClick).mousedown(slotSelectionMousedown);
    }
...
function DayEventRenderer() {
    ...
    function renderDaySegs(segs, modifiedEventId) {
        ...
        var top;
        var k;
//alert(segmentContainer.parent().html());
...
//alert(bounds.left + ":" + bounds.right);
...
function sliceSegs(events, visEventEnds, start, end) {
    ...
    for (i=0; i<len; i++) {
        ...
        var dateFormat = "yyyy-MM-dd HH:ss:ss";
//alert("起始时间：" + formatDate(eventStart, dateFormat) + "\n起始时间2:" + formatDate(start, dateFormat) + "\n结束时间1：" + formatDate(eventEnd, dateFormat) + "\n结束时间2：" + formatDate(end, dateFormat) + "\n结果：" + (eventEnd > start && eventStart < end) + "\n结果：" + (+eventEnd > +start && +eventStart < +end));
        //计划修改if (+eventEnd > +start && +eventStart < +end)
...
/*  var tmpS = [];
    tmpS.push("includeMargins" + ":"+ includeMargins);
    tmpS.push('paddingLeft' + ":" + $.curCSS(_element, 'paddingLeft', true));
    tmpS.push('paddingRight' + ":" + $.curCSS(_element, 'paddingRight', true));
    tmpS.push('borderLeftWidth' + ":" + $.curCSS(_element, 'borderLeftWidth', true));
    tmpS.push('borderRightWidth' + ":" + $.curCSS(_element, 'borderRightWidth', true));
    alert(tmpS.join("\n"));*/
...
function setMinHeight(element, h) {
    //Added by WangYanCheng ;height:auto !important;
    h = typeof h == 'number' ? h + 'px' : h;
    element[0].style.cssText += ';min-height:' + h + ';_height:' + h + ";";//";height:auto !important;";
}
