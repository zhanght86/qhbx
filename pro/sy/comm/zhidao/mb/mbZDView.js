 ...
 * 
 * */
mb.vi.zd = function(options) {
	var defaults = {
		"id":options.sId + "-mbZDView",
		"sId":"",//服务ID
		"aId":"", //操作ID
		"pCon":null,
		"pId":null,
 ...
 * 渲染列表主方法
 */
mb.vi.zd.prototype.show = function( k) {
	//this._initMainData();
	this._layout();
	this._query(k);
	this._bldGrid();
...
mb.vi.zd.prototype._initMainData = function() {
	this._query();
};
/*
 * 刷新
 */
mb.vi.zd.prototype.refresh = function() {
	var _self = this;
	this.input.val("");
	this.btn.click();
};
 ...
 * 构建列表页面布局
 */
mb.vi.zd.prototype._layout = function() {
	var _self = this;
 ...
 * 构建列表页面布局
 */
mb.vi.zd.prototype._bldSearchBar = function() {
	var _self = this;
	//默认布局
...
//    	history.go(-1);
	...
	jQuery("<div style='width:100%;text-align:center;'></div>").text("软虹知道").addClass("mbTopBar-title").appendTo(this.searchBar);
};
/*
 * 构建分类区
 */
mb.vi.zd.prototype._bldSeperateArea = function() {
	var _self = this;
	...
	jQuery("<li class='doc_nav_li' wid='25%' tpe='order'><a href='#'>提问</a></li>").appendTo(this.sepUl);
	...
	jQuery(".doc_nav_li").bind("click",function(event) {
		...
		if (type == "per") {
			...
			_self._openLayer("/cms/SY_COMM_CMS_TMPL/3O0F5DwFV5yoWFL5CwR_yb.html");
...
/**ajax search */
mb.vi.zd.prototype._searchQuery = function(){
	var _self = this;
...
mb.vi.zd.prototype._ajaxQuery = function(userCode,order,channelId){
	var _self = this;
	_self.nowPage = 1;
...
mb.vi.zd.prototype._catalogList = function(userCode,order){
	var _self = this;
    ...
    var data = FireFly.getDict("SY_COMM_ZHIDAO_CHNL_MANAGE",null," AND 1=1");
    data = data[0].CHILD;
    data = data[0].CHILD;
	var trs = [];
	//默认布局
...
mb.vi.zd.prototype._bldCatalogTr = function(node) {
	var _self = this;
	//默认布局
	var tr = [];
 ...
 * order:排序字段
 */
mb.vi.zd.prototype._query = function(userCode,order,channelId){
	var keywords = "";
	var inputVal = "";
	...
	this.data = FireFly.doAct("SY_COMM_ZHIDAO", "getAskList", data);
    this._DATA = this.data._DATA_;
};
/*
 * 构建列表页面布局
 */
mb.vi.zd.prototype._bldGrid = function() {
	var _self = this;
	this.listContainer.empty();
	this._table = jQuery("<table style='display:none;'></table>").attr("id",this._id).addClass("mbSearch-grid").appendTo(this.listContainer);
...
mb.vi.zd.prototype._bldTrs = function(data) {
	var _self = this;
    var trs = [];
	//默认布局
	var i = 0;
...
mb.vi.zd.prototype._bldTr = function(node) {
	var _self = this;
	//默认布局
	var tr = [];
	tr.push("<tr class='mbGrid-tr' uid='" + node["Q_ID"] + "'>");
	var i = 0;
	var tdLeft = [];
	tdLeft.push("<td class='mbGrid-td-left'>");
	tdLeft.push("<a class='mbGrid-doc-img '><img src='" + node["S_USER__IMG"] + "' width='40px' height='40px'></a>");//第一行
	tdLeft.push("</td>");
	var tdRight = [];
	tdRight.push("<td class='mbGrid-td-right'");
	...
	if (node["Q_TITLE"]) {//标题
		one.push("<span class='mbGrid-td-weight'>");
		var tit = node["S_USER__NAME"] + "：" + node["Q_TITLE"];
//		if (tit.length > 9) {
//			tit = tit.substring(0,10) + ".."
//		}
		one.push(tit);
	...
	if (node["Q_CONTENT"]) {//摘要
		two.push("<span class='mbGrid-td-span'>");
		two.push(node["Q_CONTENT"]);
...
//		two.push("大小：" + _self._getDocSize(node["DOCUMENT_FILE_SIZE"]));
		two.push("回答:" + node["Q_ANSWER_COUNTER"]);
		two.push(" | ");
		two.push(node["S_ATIME"].substring(0,10));
		two.push("</span>");		
	}
...
mb.vi.zd.prototype._openLayer = function(url) {
	var readOnly = false;//this.readOnly;
	//this.url = FireFly.getContextPath() + "/sy/comm/zhidao/mb/mbZDCardView.jsp?sId=&pkCode=" + uid;	
	this.url = FireFly.getContextPath() + url;	
	window.location.href = this.url;
};
/*
 * 构建列表页面布局
 */
mb.vi.zd.prototype.morePend = function(options) {
	var _self = this;
...
mb.vi.zd.prototype._getUnId = function(id) {

};
/*
 * 构建翻页
 */
mb.vi.zd.prototype._bldPage = function() {
	var _self = this;
	var res = this._DATA + "";
 ...
 * 构建翻页
 */
mb.vi.zd.prototype._bldMore = function() {
	var _self = this;
 ...
 * 加载完毕提示
 */
mb.vi.zd.prototype._recordOverTip = function() {
	var _self = this;
 ...
 * 加载完毕提示
 */
mb.vi.zd.prototype._replaceKey = function(allValue) {
	var _self = this;
 ...
 * 构建翻页
 */
mb.vi.zd.prototype.getBlocks = function() {
	var _self = this;
 ...
 * 构建翻页
 */
mb.vi.zd.prototype.trClick = function() {
	var _self = this;
	_self.listContainer.delegate(".mbGrid-tr","mousedown",function(event) {
    	...
    	if (node.hasClass("mbGrid-tr-catalog")) {
    		...
    		var url = "/cms/SY_COMM_ZHIDAO_QUESTION/" + node.attr("uid") + ".html";
			_self._openLayer(url);
		    return false; 
    	}
	});
};
 ...
 * 加载后执行
 */
mb.vi.zd.prototype._afterLoad = function() {
	var _self = this;
 ...
 * 获取文档类别
 */
mb.vi.zd.prototype._getDocType = function(stuff) {
	var _self = this;
 ...
 * 获取文档类别
 */
mb.vi.zd.prototype._getDocSize = function(size) {
	var _self = this;
	var retSize = "";
	size = size/1024;
