	...
	var backgroundChange = function(){
		...
		jQuery(".js_question_list").toggle(
			function(){
				...
				var ask_num_str = "";
				if(ask_num > 0){
					ask_num_str = "查看该主题的全部"+ask_num+"个回复";
				}else{
					ask_num_str = "查看该主题";
				}
				ask_div.append(
...
jQuery(document).ready(function(){
	...
	jQuery("#ask-type").unbind("click").live("click",function(event){
		...
		//去掉是边框变白的样式
		parent.jQuery("#SY_COMM_ZHIDAO_CHNL_MANAGE-dictDialog").next().removeClass("ui-widget-content");
	...
	jQuery("#sub-btn").unbind("click").live("click",function(){
		...
		if (!jQuery("#questionIdHid").val()) {
			if (quesitionTtle && quesitionTtle!='出题' && quesitionTtle!='写下你的主题') {
				if (askType) {
...
//将出题页面整体使用js加载进去
	var loadTiWen = function(person_name,person_id) {
		//没有求助这个功能了，所以求助人和ID就没用了
		var options = {"url":"SY_COMM_ZHIDAO_QUESTION_HTML.card.do","tTitle":"出题","params":null,"menuFlag":3};
		Tab.open(options);
	}
//var loadTiWen = function(person_name,person_id){
//	var topPt = 20;
//	try{
//		var mousePt = Mouse.getMousePoint(event);
//		topPt = parseFloat(mousePt["y"]) - parseInt(250);
//	}catch(e){
//		alert("当前的坐标为：" + topPt);
//	}
...
//		'<div aria-labelledby=":4e" role="dialog" style="width: 550px; left: 398.5px; top: ' + topPt + 'px;" tabindex="0" class="modal-dialog absolute-position">'+
//		  '<div id=":4e" class="modal-dialog-title modal-dialog-title-draggable">'+
//		  	'<span class="modal-dialog-title-text">出题</span>'+
//		  	'<span class="modal-dialog-title-close" style="height:25px;width:25px;"></span>'+
//		  '</div>'+
...
//		            '<div class="zm-modal-dialog-warnmsg zm-modal-dialog-guide-warn-message zg-r5px">您还没有给主题添加问号</div>'+
...
//		            '<textarea aria-autocomplete="list" role="combobox" aria-activedescendant="" style="white-space: pre; height: 22px; min-height: 22px;" autocomplete="off" aria-haspopup="true" placeholder="写下你的主题" aria-label="写下你的主题" class="zg-editor-input zu-seamless-input-origin-element" title="在这里输入主题" id="zh-question-suggest-title-content"></textarea>'+
//		          '</div>'+
//		          '<span style="display: none;" id="js-title-length-err-msg" class="title-length-err-msg zg-right">主题字数太少了吧</span>'+
...
//		              '<div style="-moz-user-select: none;" class="ac-head zg-gray">你的主题可能已经有答案</div>'+
//		            '</div>'+
//		          '</div>'+
//		        '</div>'+
...
//		                  '<div style="" g_editable="true" class="zm-editable-editor-field-element editable" id="question_desc" placeholder="写下你对主题的说明" contenteditable="true">'+
//		                  '</div>'+
//		                '</div>'+
//		              '</div>'+
...
//		          '<div class="add-question-section-title"> <span class="zg-gray zg-right">分类越精准，越容易让领域专业人士看到你的主题</span> 选择分类： <span id="zh-question-form-tag-err"></span> </div>'+
...
//		          '<a id="look_question" href="javascript:void(0);" name="jumpq" class="zg-r5px zg-btn-blue zu-question-form-jump" style="display:none;">查看主题</a>'+
//		        '</div>'+
//		      '</form>'+
//		    '</div>'+
//		  '</div>'+
