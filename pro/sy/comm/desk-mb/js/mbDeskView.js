...
mb.vi.deskView.prototype._bldOneApp = function(item) {
   ...
   block.bind("mousedown",function() {
	   var opts = {};
	   if (type == 1) { // 服务
		   var sid = info.split(".")[0];//截取正确的服务id
		   //打开一个dialog
		   var extWhere = "";
   ...
   if ((info == "SY_COMM_TODO") || (item.ALERT && item.ALERT.length > 0)) {
...
mb.vi.deskView.prototype._openLayer = function(opts) {
	...
	if (type == 1) { // 服务
		...
		var info = opts["info"];
		if (info.toUpperCase().indexOf("HTTP://") == 0) {
			this.url = info;
		} else {
			if (info.indexOf("/") == 0) { // 相对地址，以'/'开头
				this.url = FireFly.getContextPath() + info;
			} else if(info.indexOf(".card.do") >= 0) { //相对地址，以'.card.do'开头 
				this.url = FireFly.getContextPath() + "/sy/base/view/stdCardView-mb.jsp?sId=" + info.replace(".card.do","");
			}else {
				this.url = FireFly.getContextPath() + "/" + info;
			}
		}
