...
function openKMCard(id,name) {
		var opts = {"tTitle":name,"url":"SY_COMM_INFOS_VIEW.card.do?pkCode=" + id,"menuFlag":3};
		Tab.open(opts);
}
function openKMList(CHNL_ID,title) {
		...
		var opts = {"tTitle":title,"url":"SY_COMM_INFOS_VIEWMORE.list.do","params":params,"menuFlag":3};
		Tab.open(opts);
}
</script>
<style type="text/css">
