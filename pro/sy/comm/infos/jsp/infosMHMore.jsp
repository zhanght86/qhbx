...
<%
	...
	if(outData.contains("_INFOS_PARAM_")){
	 outBean= (OutBean)outData.getBean("_INFOS_PARAM_");
	}
	String pttype = outBean.getStr("PT_TYPE").trim();
...
<table width="100%" border="0"  cellpadding="2" cellspacing="0">
 <tr>
    <td height="20" align="left" class="page_top_menu_grey"><a href="SY_COMM_TEMPL.show.do?pkCode=3ziU0Bi5la0G_Jcvrr73ac"><img src="/sy/comm/infos/img/btn_index.jpg" border="0" align="absmiddle"></a> 
      &gt;&gt;<span style="font-size:12px;">分公司新闻门户列表</span>
      </td>
  </tr>
  </table>
...
<td class="elipd" style="width:75%;"><img src="/sy/comm/infos/img/d.png"/><a href="javascript:void(0);" onclick="newsMHview('<%=bean.getStr("PT_TITLE") %>','<%=bean.getStr("PT_ID")%>')"  style="margin-left:3px;">
<span style="font-size:12px;"><%=bean.getStr("PT_TITLE")%></span>
</a></td>
<td class="elipd" style="width:25%;"><span style="float:right;margin-right:30px;color:#999999;font-size:12px;"><%=bean.getStr("S_MTIME").substring(0, 10) %></span></td>
</tr>
<%}%>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
...
if(countPage>1&&currentPage!=1)
	document.write("<a href='SY_COMM_INFOS.getMoreMhTmpl.do?PT_TYPE=<%=pttype%>&curtentpage="+prevPage+"' onMouseOver=\"pageup.src='/sy/comm/infos/img/page_up_currected.jpg'\"  onMouseOut=\"pageup.src='/sy/comm/infos/img/page_up.jpg'\"><img src=/sy/comm/infos/img/page_up.jpg border=0 name=pageup>&nbsp;<span class=font_c_black>上一页</span></a>&nbsp;");
else
...
for(var i=0+(currentPage-(currentPage-1)%num) ; i<=(num+(currentPage-(currentPage-1)%num))&&(i<=countPage) ; i++){
	if(currentPage==i)
		...
		document.write("&nbsp;<a href='SY_COMM_INFOS.getMoreZT.do?PT_TYPE=<%=pttype%>&curtentpage=" +i+ "'>" + (i) + "</a>&nbsp;|");
}
//设置下一页代码 
if(countPage>1&&currentPage!=(countPage))
	document.write("&nbsp;<a href='SY_COMM_INFOS.getMoreZT.do?PT_TYPE=<%=pttype%>&curtentpage="+nextPage+"' onMouseOver=\"pagedown.src='/sy/comm/infos/img/page_down_currected.jpg'\"  onMouseOut=\"pagedown.src='/sy/comm/infos/img/page_down.jpg'\">&nbsp;<span class=font_c_black>下一页</span>&nbsp;<img src=/sy/comm/infos/img/page_down.jpg width=12 height=12 border=0 name=pagedown></a>&nbsp;");
else
