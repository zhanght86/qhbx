...
rh.cm.MindInput = function(name) {
	...
	this.clearContent = function(){
		...
		//文件上传组件已经注释掉了，所以这里不用清理文件。清理会报错，导致下面的程序无法执行
		//this.file.clear();
		this._fileIsModify = false;
		this.mindId = "";
		this.oldMindText = "";
...
rh.cm.mind.prototype.render = function() {
	var _self = this;
	//如果是分发、签收用户则不能查看意见
	if(_self._wfCard.getDisplayMode() == "MODE_SEND"){
		var showMindMode = System.getVar("@C_CM_MIND_SHOW_EVERYTIME@");
		if (showMindMode != "TRUE") {
			return;
		} 
	}
	_self._setDefaultSortType(); //分析意见的默认显示类型
	if (_self._wfCard) {

		_self.getMindData(undefined,function(){
			// 意见容器
			...
			if (jQuery("#saveMindBtn").length > 0) {
				...
				if(_self._generalMind && _self._generalMind.oldMindText==""){
...
*/
					...
					if(saveResult){
						//_self.refresh();
						alert("意见保存成功，请点击'提交任务'下的节点按钮继续送交!!!");
...
rh.cm.mind.prototype._bldUsualMind = function(mind){
	...
	if (this.isMobile) { //移动版
		...
		commTag = jQuery("<span class='fl ml30 mt5 unl cp' style='font-size:14px;margin-top:5px;margin-right:30px'>常用批语</span>");
		$("#saveMindBtn").after(commTag);
...
rh.cm.mind.prototype.saveGeneralMind = function() {
	...
	return _self._saveMind(data,_self._generalMind);
};
...
rh.cm.mind.prototype._saveMind = function(data,mindInputObj) {
	var _self = this;
	if(!data || !data.MIND_CODE){
		alert("参数错误，请检查！" + JsonToStr(data||{}));
		this._viewer.cardBarTipError("参数错误，请检查！" + JsonToStr(data||{}));
		return false;
	}
	var resultData = FireFly.doActByGet("SY_COMM_MIND", "save", data,false,false);
	//if("lidandan"==System.getVar("@LOGIN_NAME@")){
	//	resultData = FireFly.doActByGet("SY_COMM_MIND", "save", data,false,false);
	//}else{
	//	resultData = FireFly.doAct("SY_COMM_MIND", "save", data,false,false);
	//}
	...
	if (resultData[UIConst.RTN_MSG] && resultData[UIConst.RTN_MSG].indexOf(UIConst.RTN_OK) == 0) {
		...
		alert("返回错误，请检查！" + JsonToStr(resultData)+";;前端参数："+JsonToStr(data||{}));
		this._viewer.cardBarTipError("返回错误，请检查！" + JsonToStr(resultData)+";;前端参数："+JsonToStr(data||{}));
		return false;
	}
};
...
rh.cm.mind.prototype.saveAgreeSendMind = function(mindText){
	...
	if (mindTextVal == "") {
		if (!jQuery("#GENERAL_MIND").is("textarea")) {
			_self._generalMind.setMindTextInput(jQuery("#GENERAL_MIND"));
		}
...
rh.cm.mind.prototype.displayMindList = function(odeptCode, sortType) {
	var _self = this;
	var res = _self._getMindData(sortType, odeptCode, "listMind",function(res){
...
rh.cm.mind.prototype._copyMind = function() {
	...
	jQuery(".mindTable").find("a.COPY_MIND").unbind("click").bind("click", function(){
		...
		//点击复制后 直接复制 不用选择意见类型  所以注释掉一些代码 并作小小修改 Aodanni 2015-09-23
//		var html = new Array();
 ...
 * 获取意见数据
 */
rh.cm.mind.prototype.getMindData = function(sortType,callback) {
	var _self = this;
	var res = this._getMindData(sortType,undefined,undefined,function(res){
...
rh.cm.mind.prototype.refresh = function(sortType) {
	...
	_self.getMindData(sortTypeTemp,function(){
