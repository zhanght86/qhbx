...
rh.vi.gwHeader.prototype.createYearCodeItem = function() {
	...
	// 流程节点没有设置机关代字控制变量，则隐藏文号行
	
	if (!this.canLoadGwCode()) {
		if (opts._viewer.getByIdData("GW_YEAR_CODE") != "") { //表单中已将有值
			
		}else{
			if(_viewer._servPId=="OA_GW_TYPE_SW" && _viewer.byIdData["_ADD_"] == "true"){

			}else{
				var gwCodeTr = jQuery("#GWCODE-TR");
				if (gwCodeTr) {
					gwCodeTr.css({"display":"none"});
					_viewer.form.setNotNull("GW_YEAR_CODE",false);
					_viewer.form.setNotNull("GW_YEAR",false);
					return;
				}
			}
		}
	...
	if(yearCode.type.toLowerCase() == "select"){ //代字输入框为Select
		...
		if (opts._viewer.getByIdData("GW_YEAR_CODE") != "") { //表单中已经有值
...
//			this.selectYearCodeOption(yearCode,opts.defValue);
			yearCode.setValue(selectedVal,selectedVal);
		} else if(_viewer._servPId=="OA_GW_TYPE_SW" && _viewer.byIdData["_ADD_"] == "true"){
...
function getMaxCode(_viewer, yearCode, compareFlag) { // 获取机关代字最大值
	...
	//收文的添加模式主键取GW_ID。其他取pkCode
	if(_viewer._servPId=="OA_GW_TYPE_SW" && _viewer.byIdData["_ADD_"] == "true"){
		param[UIConst.PK_KEY] = _viewer.getItem("GW_ID").getValue();
	}else{
