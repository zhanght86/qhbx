...
rh.vi.gwExtCardView.prototype.init = function() {
	...
	if (this.wfCard != null) {
		this.wfCard.beforeFenfa = function(fenfaObj){
			fenfaObj["SCOPE"] = "ODEPT";
		}
...
rh.vi.gwExtCardView.prototype.seal = function(event, actItem) {
	...
	if(sealType == "SHUSHENG"){ //书生盖章功能
		...
		var sealParams = self.getSealFileInfo();
		
		if(sealParams._MSG_ != undefined && StringUtils.startWith(sealParams._MSG_ ,"ERROR,")){
			alert(sealParams._MSG_);
			return;
		}else if(sealParams.ODEPT_NUM == ""){
			alert("机构编号不能为空，请检查机构编号或与管理员联系。");
			return;
		} else if(sealParams.SEAL_CODE == undefined || sealParams.SEAL_CODE == ""){
			alert("机关代字对应的印章不能为空，请先设置代字对应的印章。");
			return;
		}
		
		var oaHost = sealParams.SYS_HOST_ADDR;
		
		var urlStr = sealParams.SEAL_SYS_HOST + "/general/modules/wendang/anonDoc/sealShow.jsp" 
				   + "?sealIds=" + sealParams.SEAL_CODE
				   + "&userName=" + sealParams.USER_LOGIN_NAME
				   + "&sealFilePath=" + oaHost + sealParams.FILE_PATH
				   + "&printNum=" + sealParams.DEFAULT_PRINT_NUM
				   + "&wyh=" + sealParams.zhengWenId
				   + "&rtnUrl=" + oaHost + "/oa/zh/returnSealFile.jsp"
				   + "&sealFileName=" + (self.parHandler.getByIdData("GW_TITLE"))
				   + "&printDeptId=" + sealParams.ODEPT_NUM;
		
		jQuery(document).data("parHandler", this.parHandler);
...
rh.vi.gwExtCardView.prototype.undoSeal = function(event, actItem) {
	...
	try{
		self.wfCard.getParHandler().shield();
		var result = FireFly.doAct(servId, "undoSeal", reqdata);
		if (result.rtnMsg == "succ") {
			self.wfCard.getParHandler().refresh();
			alert("取消成功！");
		}
	} catch (e) {

	} finally{
		self.wfCard.getParHandler().shieldHide();
	}
}
 ...
 * 公文管理
 */
rh.vi.gwExtCardView.prototype.fileGuanLi = function(event, actItem) {
...
rh.vi.gwExtCardView.prototype.printZw = function(event,actItem){
	...
	if(result.SEAL_FILE_ID){
		var urlStr = result.SEAL_SYS_HOST
				+ "/general/modules/wendang/printDoc/oa_print.jsp"
				+ "?wyh=" + result.GW_ID
				+ "&saveNo=" + result.SEAL_FILE_ID
				+ "&printUserName=" + result.USER_LOGIN_NAME
				+ "&printDeptId=" + result.ODEPT_NUM;
...
rh.vi.gwExtCardView.prototype._getPrintFileInfo = function(){
	...
	var result = FireFly.doAct(formServId, "getPrintFileInfo", reqdata);
