<style>
	...
	.current {padding:4px 10px 4px 10px;border:#8db5d7 1px solid;color:white;margin:0 2px 0 2px;background-color:#0080FF;cursor:pointer;}
	.ui-corner-4 {-moz-border-radius:4px 4px;-webkit-border-radius:4px 4px;-o-border-radius:4px 4px;-ms-border-radius:4px 4px;-khtml-border-radius:4px 4px;border-radius:4px 4px;}
	a:hover {padding:4px 10px 4px 10px;background-color:#0080FF;margin:0 2px 0 2px;}
	a:active {padding:4px 10px 4px 10px;background-color:#0080FF;margin:0 2px 0 2px;}
	span {display:inline-block;}
	span.current {padding:4px 10px 4px 10px;border:#8db5d7 1px solid;color:white;margin:0 2px 0 2px;background-color:#0080FF;}
	span.disabled {padding:4px 10px 4px 10px;border:#ccc 1px solid;color:#ccc;margin:0 2px 0 2px;}
</style>
<#--引用外部工具函数-->
		...
		<div id='LDR-LIST'>
			...
			<div>
				<table id="LDR-LIST-tabHead">
					<thead>
						<tr>
							<td width=80% style='font-size:16px;'>角色列表</td>
						</tr>
					</thead>
				</table>
			</div>
			...
			<div id='ACTION-TAB'>
				...
				<div id='WEEK-CON' class='portal-box' style="border-top:0px solid black;border:0px;">
					...
					<div id='week-action-con' style='height:auto;width:100%;OVERFLOW:auto;'>
						<table width="915px" border="1" style='border:1px #CCC solid;'>
							<!--表格标题-->
					...
					<!--下方注释 start-->
					<div>
						<table width="50%" border="1" id="note" >
						  <tr>
						    <td width="20%" align="center"><font  style='font-weight:bold'>图&nbsp;例</font></td>
						    <td> (注明:缺省都表示"日常")</td>
						  </tr>
						  <tr>
    	 ...
    	 FireFly.doAct("BN_LDR_QUERY_USER", "getLeaderList", {"ROLE_CODE":roleCode,"SHOWNUM":"10","NOWPAGE":"1"},false,false,function(result){
   			if(result._DATA_.length==0){
				...
				if(Number(result._PAGE_.ALLNUM)>Number(result._PAGE_.SHOWNUM)){
					...
					<#--如果当前显示页为首页 -->
					jQuery("<span id='up' class='current ui-corner-4'>上一页</span>").appendTo(jQuery("#fenye")).click(function(){
					...
					for(var n=1;n<=Number(result._PAGE_.PAGES);n++){
						<#--如果当前显示页为第n页-->
			   			jQuery("<span id='num"+n+"' class='current ui-corner-4'>"+n+"</span>").attr("data",n).appendTo(jQuery("#fenye")).click(function(){
				   	...
				   	<#--如果当前显示页为最后一页 -->
					jQuery("<span id='down' class='current ui-corner-4'>下一页</span>").appendTo(jQuery("#fenye")).click(function(){
						var nowPage = parseInt(jQuery("#nowPage").val());
						FireFly.doAct("BN_LDR_QUERY_USER", "getLeaderList", {"ROLE_CODE":roleCode,"SHOWNUM":"10","NOWPAGE":nowPage+1},false,false,function(result){
   							<#-- 执行渲染action的方法 -->
   							jQuery("#nowPage").val(result._PAGE_.NOWPAGE);
		...
		for(var i=0;i<result._DATA_.length;i++){
			...
			if(actionLists._DATA_.length>0){
				for(var j=0;j<actionLists._DATA_.length;j++){
					...
					var floatLeft = 2*(parseInt(actionData.BEGIN_BLOCK)-1);
					var dataTd = jQuery("#tr"+leaderList.USER_CODE+" #actionTd");
					jQuery(dataTd).append("<div id='div"+j+"' style='background-color:"+tdcolor+";width:"+divW+"px;height:30px;overflow:hidden;left:"+floatLeft+"px;position:absolute;float:left'></div>");
