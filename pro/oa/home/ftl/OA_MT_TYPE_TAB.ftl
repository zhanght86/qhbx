...
<div id='OA_MT_TYPE_TAB' class='portal-box tabs-portal ${boxTheme}' style='height:${height};min-height:200px'>
...
jQuery(document).ready(function(){
    setTimeout(function() {
      jQuery("#con_ul1").find("iframe").attr("src","SY_COMM_INFO.chart.do?_PK_=OA_MT_MEETING_COUNT&BEGIN_DATE="+rhDate.pattern("yyyy-MM")+"&END_DATE="+rhDate.pattern("yyyy-MM"));
      jQuery(".new_tab").bind("mouseover",function() {
	      ...
	      thisObj.find("iframe").attr("src","SY_COMM_INFO.chart.do?_PK_=OA_MT_MEETING_COUNT&BEGIN_DATE="+dateObj["BEGIN_DATE"]+"&END_DATE="+dateObj["END_DATE"]);
	  });
    },0);
});
</script>
