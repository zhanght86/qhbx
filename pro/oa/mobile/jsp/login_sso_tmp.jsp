...
<%
	...
	String userLoginName = "";//登陆号
	if (userCode.isEmpty()) { // 从request中获取
		userCode = request.getParameter("userCode") == null ? "" : request.getParameter("userCode").toString();
	}
	if (userLoginName.isEmpty()) { // 从request中获取
		userLoginName = request.getParameter("userLoginName") == null ? "" : request.getParameter("userLoginName").toString();
	}
	String homeUrl = "";
	Bean user = new Bean();
	Map orgMap = null;
	if (userCode.isEmpty()) { // 如果为空，说明Cookie中没有取到值
		...
		if (!"".equals(userCode)) {
			...
			homeUrl = "/oa/mobile/jsp/WxSso.jsp";
		} else {
			// TODO 如果不支持除微信以外的浏览器，直接关闭页面
